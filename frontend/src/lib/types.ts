...
export interface TaskBoardItem {
  ...
  observability?: Record<string, unknown> | null;
  gate_results: TaskGateResultSummary[];
...
export interface OptimizationSummary {
  ...
  chunk_pressure?: {
    available: boolean;
    runs_with_signal: number;
    risky_runs: number;
    large_output_runs: number;
    large_event_runs: number;
    largest_event_bytes: number;
    largest_command_output_bytes: number;
    chunk_pressure_risk: boolean;
  };
...
export interface MetricsData {
  ...
  voice_ledger?: {
    ...
    delegated_messages?: Array<Record<string, unknown>>;
    ...
    totals?: {
      ...
      voice_digests?: number;
      tool_calls?: number;
      tool_outputs?: number;
      wait_events?: number;
      prepared_actions?: number;
...
export interface ContextBudgetSummary {
  ...
  by_lane?: Array<{ lane: string; count: number }>;
  by_stage?: Array<{ stage: string; count: number }>;
  signal_counts?: Array<{ signal_category: string; count: number }>;
  top_components?: Array<{ name: string; estimated_tokens: number }>;
  latest?: {
    lane?: string;
    stage?: string;
    runtime_sdk?: string;
...
export interface RuntimePreferenceState {
  designTheme: "calm" | "operator" | "sage" | "ember" | "midnight" | "paper";
  designMode: "preset" | "light" | "dark";
  designAccentHue: number | null;
  designAccentChroma: number | null;
  designDensity: number | null;
  designRadius: number | null;
  designDisplayFace: "preset" | "Newsreader" | "Geist";
  realtimeModel: "gpt-realtime-mini" | "gpt-realtime";
  realtimeVoice: "alloy" | "verse" | "cedar";
  pushToTalkMode: "hold" | "toggle";
  inlineTranscript: "on" | "off";
  bindVoiceToCurrentSession: "on" | "off";
  destructiveActionPhrase: "typed-confirmation" | "required-before-dispatch";
  agentDefaultMode: "chat" | "trace";
  ...
  runTraceDefault: "thread" | "raw" | "full";
