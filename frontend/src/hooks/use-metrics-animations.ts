...
export function useMetricsAnimations() {
  ...
  useGSAP(
    () => {
      ...
      const kpiCards = gsap.utils.toArray<HTMLElement>("[data-kpi]");
      if (kpiCards.length) {
        gsap.from(kpiCards, {
          scrollTrigger: {
            trigger: kpiCards[0],
      ...
      const bars = gsap.utils.toArray<HTMLElement>("[data-cost-bar]");
      bars.forEach((bar, i) => {
      ...
      const rows = gsap.utils.toArray<HTMLElement>("tbody tr");
      if (rows.length) {
        gsap.from(rows, {
          opacity: 0,
          x: -8,
