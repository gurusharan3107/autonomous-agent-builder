...
import type { OnboardingStatus, RuntimeSdk, RuntimeSettings } from "@/lib/types";
import {
  Meter,
  PageFrame,
  PageHeader,
...
export default function OnboardingPage({
  ...
  useLayoutEffect(() => {
    ...
    gsap.fromTo(
      active,
      { scale: 0.985, boxShadow: "0 0 0 oklch(0 0 0 / 0)" },
      {
        scale: 1,
  ...
  return (
    <PageFrame className="pt-2" data-screen-label="Onboarding">
      <div
        ref={shellRef}
        className="surface-elevated relative overflow-hidden rounded-[var(--radius-lg)] border border-border/70 px-4 py-4 sm:px-5"
      >
          ...
          <SurfacePanel className="space-y-4 border-border/70 bg-background/60">
            ...
            <div className="rounded-[1.35rem] border border-border/70 bg-background/68 p-4">
              <div className="flex items-center justify-between gap-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Runtime SDK</p>
                <span className="font-mono text-[11px] text-muted-foreground">
                  {runtimeSettings?.model ?? "loading"}
                </span>
              </div>
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {[
                  { label: "Claude Agent SDK", value: "claude" as RuntimeSdk },
                  { label: "Codex SDK", value: "codex_sdk" as RuntimeSdk },
                ].map((option) => (
                  <Button
                    key={option.value}
                    type="button"
                    variant={runtimeSettings?.sdk === option.value ? "default" : "outline"}
                    aria-pressed={runtimeSettings?.sdk === option.value}
                    className="justify-start rounded-full"
                    disabled={runtimePending || actionPending || running}
                    onClick={() => onRuntimeSelect(option.value)}
                  >
                    {option.label}
                  </Button>
                ))}
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-muted-foreground">
                ...
                <span className="rounded-full border border-border/70 px-2.5 py-1">
                  {runtimeSettings?.telemetry?.active_lane ?? "telemetry"}
                </span>
              </div>
            </div>
