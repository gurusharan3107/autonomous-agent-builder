...
export default function AgentPage() {
  ...
  const [effortName, setEffortName] = useState<string | null>(null);
  const [runtimeSdk, setRuntimeSdk] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      const loadedHistory = await loadHistory(storedSessionId);
  ...
  }, [searchParams, selectedRuntimeSdk, sessionStorageKey]);
  ...
  useEffect(() => {
    ...
    return undefined;
  }, [items, streamingText, pendingBlockingItemId]);
  ...
  const currentRunPanel = (
    <SurfacePanel data-agent-stage="card" className="space-y-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
        ...
        <MetricRow label="Selected runtime" value={selectedRuntimeSdk ?? status?.runtime_sdk ?? runtimeSdk ?? "loading"} mono />
        <MetricRow label="Thread runtime" value={status?.runtime_sdk ?? runtimeSdk ?? "new thread"} mono />
        <MetricRow
          label="Cost"
          value={runtimeCostDisplay(
            status?.cost_usd,
            status?.runtime_sdk ?? runtimeSdk,
        ...
        <div className="rounded-[1.5rem] border border-border/80 bg-background/70 p-3">
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            <Button
              ...
              disabled={!input.trim() || loading || Boolean(pendingBlockingItem)}
            >
              Send
            </Button>
          </div>
            ...
            <div className="sticky top-20 space-y-3">
              ...
              >
                ...
                <div className="relative space-y-2">
                  ...
                  <MetricRow label="Selected runtime" value={selectedRuntimeSdk ?? status?.runtime_sdk ?? runtimeSdk ?? "loading"} mono />
                  <MetricRow label="Thread runtime" value={status?.runtime_sdk ?? runtimeSdk ?? "new thread"} mono />
                  <MetricRow label="Selected provider" value={selectedProviderName ?? providerName ?? "loading"} mono />
                  <MetricRow label="Effort" value={effortName ?? "loading"} mono />
                  <ProgressMeter
