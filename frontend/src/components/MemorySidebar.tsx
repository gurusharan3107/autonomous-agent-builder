...
> = {
  decision: { label: "Decision", className: "border-accent/35 bg-accent/10 text-accent" },
  pattern: { label: "Pattern", className: "border-border/80 bg-background/55 text-foreground/80" },
  correction: {
    label: "Correction",
    className: "border-status-blocked/30 bg-status-blocked-soft text-status-blocked",
  },
};
        ...
        <div className="detail-drawer-panel space-y-6">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <Code className={`text-[10px] uppercase tracking-[0.18em] ${style.className}`}>
                  {style.label}
                </Code>
                {entry.status && (
          ...
          {entry.tags && entry.tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {entry.tags.map((tag) => (
                <Code key={tag} className="text-[10px] uppercase tracking-[0.14em]">
                  {tag}
                </Code>
              ))}
            </div>
