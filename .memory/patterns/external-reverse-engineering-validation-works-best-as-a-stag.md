...
Retrieve this memory when working on reverse-engineering-lifecycle, testing, or related pattern changes. Use it to preserve the repo-local precedent: Summary For reverse-engineering validation on an external repo, the most reliable sequence is: clone into a clean temp repo, run builder init, complete onboarding until ready: true, verify deterministic KB extraction bef

## User-Facing Summary

Summary For reverse-engineering validation on an external repo, the most reliable sequence is: clone into a clean temp repo, run builder init, complete onboarding until ready: true, verify deterministic KB extraction bef

## Reusable Guidance

- Treat this as repo-local pattern precedent for reverse-engineering-lifecycle.
...
Apply this when changes touch reverse-engineering-lifecycle, the testing phase, or related tags: reverse-engineering, testing, embedded-server, external-repo.

## Retrieval Queries

- external reverse-engineering validation works best as a staged runtime drill
- external reverse engineering validation works best as a stag
