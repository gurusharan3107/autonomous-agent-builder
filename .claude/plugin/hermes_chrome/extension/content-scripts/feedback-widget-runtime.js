(() => {
  ...
  function ensureThreadFromServerItem(item) {
    ...
    // Don't resurrect threads the operator deleted (server flips item.status to
    // "canceled" when all messages are removed). Without this guard, pollStatus /
    // SSE rebroadcasts re-create the marker 2-3s after the trash click.
