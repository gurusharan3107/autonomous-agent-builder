...
def preflight_or_skip():
    ...
    if not needed.issubset(nav):
        print(
            f"SKIP: dashboard at {BASE} is not the expected Agent Builder "
...
def round1():
    ...
    check(
        "T1 click_text 'Board' → /board (not skip-link)",
        url().endswith("/board"),
    ...
    check(
        "T6 NEG missing text → error + no nav",
        r.get("success") is False and url() == before,
    ...
    seq(
        [
            {"type": "click_selector", "selector": "a[href$='/settings']"},
            {"type": "wait", "ms": 800},
        ]
    )
    check(
        "T8 click_selector dup matches → visible → /settings",
    ...
    r = seq(
        [
            {
                "type": "fill_selector",
                "selector": "input[name='memory-search']",
                "value": "decision",
            },
            {"type": "wait", "ms": 300},
            {
                "type": "evaluate",
                "expression": "document.querySelector(\"input[name='memory-search']\").value",
            },
        ]
    )
    check(
        "T9 fill_selector → value set", (r.get("results") or [{}])[-1].get("result") == "decision"
    )
...
def round2():
    ...
    if low and low.get("text"):
        ...
        if res:
            ...
            check(
                "R1 off-screen elem: scrolled + cursor landed",
                bool(landed),
    ...
    time.sleep(0.3)
    check(
        "R2 occlusion: bg nav click blocked while modal open",
        bool(modal) and blocked,
    ...
    pt = (r.get("results") or [{}])[0].get("point", {})
    check(
        "R3 'Decisions' matches 'Decisions 0' pill",
        "decision" in pt.get("text", "").lower(),
        f"matched={pt.get('text')!r}",
    )
    ...
    near = (
        abs(cs.get("x", -999) - 716) < 5 and abs(cs.get("y", -999) - 34) < 5 and cs.get("visible")
    )
    check(
        "R5 cursor_move → cursor AT (716,34) visible",
        bool(near),
    ...
    r = seq(
        [
            {"type": "click_text", "text": "Patterns"},
            {"type": "wait", "ms": 250},
            {"type": "click_text", "text": "Corrections"},
            {"type": "wait", "ms": 250},
        ]
    )
    ...
    check(
        "R6 ordered batch: Patterns then Corrections",
        "pattern" in p1.lower() and "correction" in p2.lower(),
        f"first={p1!r} second={p2!r}",
    )
    ...
    pc = (seq([{"type": "page_context"}]).get("results") or [{}])[0]
    check(
        "R7 nav→/settings then immediate page_context",
        pc.get("url", "").endswith("/settings"),
...
def round3():
    ...
    pc = (seq([{"type": "page_context"}]).get("results") or [{}])[0]
    check(
        "N1 SSE-SPA goto settles (not networkidle-hang)",
    ...
    check(
        "N2 dead-target goto bounded + failed load detectable",
    ...
    pc = (seq([{"type": "page_context"}]).get("results") or [{}])[0]
    check(
        "N3 bridge recovers after dead-target nav",
        pc.get("url", "").endswith("/memory"),
        f"url={pc.get('url')}",
    )
