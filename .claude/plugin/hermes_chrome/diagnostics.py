...
def _check(
    name: str, ok: bool | None, *, severity: str, detail: str, surface: str, fix: str
) -> dict[str, Any]:
    return {
        "name": name,
        "ok": ok,
...
def _check_socket() -> dict[str, Any]:
    ...
    try:
        ...
        return _check(
            ...
            True,
            severity="blocking",
            detail="socket present and accepting connections",
            surface="-",
            fix="-",
        )
...
def _check_extension_deployed() -> dict[str, Any]:
    ...
    required = ["manifest.json", "service_worker.js", "content-scripts/cursor-agent.js"]
    ...
    return _check(
        ...
        True,
        severity="blocking",
        detail=f"core files present at {ext}",
        surface="-",
        fix="-",
    )
...
def _check_cursor_assets() -> dict[str, Any]:
    ...
    if ext is None or not ext.exists():
        return _check(
            ...
            None,
            severity="blocking",
            detail="deployed extension dir unresolved",
            surface="diagnostics",
    ...
    assets = [a for a in assets if a.startswith("images/")] or [
        "images/cursor-agent.png",
        "images/ocean-glow-pointer.svg",
    ...
    if missing:
        return _check(
            ...
            fix="Run scripts/sync.sh; confirm the assets exist in plugin/extension/images/.",
        )
    return _check(
        "cursor_assets_present",
        True,
        severity="blocking",
        detail=f"{len(assets)} cursor assets present",
        surface="-",
        fix="-",
    )
...
def _check_drift() -> dict[str, Any]:
    ...
    return _check(
        ...
        True,
        severity="warning",
        detail="deployed extension matches source",
        surface="-",
        fix="-",
    )
...
def _check_native_manifest() -> dict[str, Any]:
    ...
    if not mani_path.exists():
        return _check(
            ...
            fix="Run install_hermes_chrome_bridge.py --extension-id <id from chrome://extensions>.",
        )
    try:
    ...
    return _check(
        ...
        True,
        severity="blocking",
        detail=f"bound to {origins[0]}, launcher present",
        surface="-",
        fix="-",
    )
...
def _check_launcher() -> dict[str, Any]:
    ...
    if win is None:
        return _check(
            ...
            None,
            severity="warning",
            detail="Windows .claude dir unresolved",
            surface="diagnostics",
            fix="-",
        )
    ...
    if "wsl" in body.lower() and "python3" in body.lower():
        if str(host) in body:
            return _check(
                ...
                True,
                severity="blocking",
                detail="launcher runs the deployed WSL native host",
                surface="-",
                fix="-",
            )
        return _check(
...
def run_diagnostics() -> dict[str, Any]:
    ...
    blocking = [c["name"] for c in checks if c["ok"] is False and c["severity"] == "blocking"]
    warnings = [c["name"] for c in checks if c["ok"] is False and c["severity"] == "warning"]
