...
/* eslint-disable */
/**
 * analyze-sessions.mjs — BUNDLED + TAILORED for the goal-audit skill.
 *
 * Original source: claude-plugins-official/session-report/skills/session-report.
 * This file is intentionally vendored so the goal-audit skill is fully self-contained
 * and does not depend on the session-report plugin being installed.
 *
 * Tailored differences from upstream:
 *   - Adds --filter-pattern <comma,separated,substrings> to drop non-matching
 *     project keys early at file-discovery time. Reduces output size, prevents
 *     accidental analysis of unrelated projects.
 *   - Adds --top-prompts <N> (default 200) to control the prompts array
 *     length independently of --top. Goal-audit benefits from more prompts
 *     to surface intent signal beyond the heaviest few.
 *
 ...
 *
 * Usage:
 *   node analyze-sessions.mjs [--dir <projects-dir>] [--json] [--since <ISO|7d|24h>]
 *                             [--top N] [--top-prompts N]
 *                             [--filter-pattern devpulse,builder-workspace,...]
 *
...
// recent_prompts is recency-ranked. The original session-report's token-weighted
// top_prompts is biased toward heavy historical prompts and buries recent short
// prompts — useless for intent analysis. The token-weighted variant was removed
// in goal-audit's tailoring; cost attribution is a separate concern.
const RECENT_PROMPTS = parseInt(flag('--recent-prompts', '500'), 10)
...
// Comma-separated substrings; if any matches the project key (case-insensitive),
// the file is processed. Empty string means no filter (process all projects).
const FILTER_PATTERNS_RAW = flag('--filter-pattern', '')
const FILTER_PATTERNS = (typeof FILTER_PATTERNS_RAW === 'string' ? FILTER_PATTERNS_RAW : '')
  .split(',')
  .map(s => s.trim().toLowerCase())
  .filter(Boolean)
function projectMatchesFilter(projectKey) {
  if (FILTER_PATTERNS.length === 0) return true
  const k = (projectKey || '').toLowerCase()
  return FILTER_PATTERNS.some(p => k.includes(p))
}
...
async function main() {
  ...
  const files = [...walk(ROOT)]
    ...
    .filter(({ info }) => projectMatchesFilter(info.project))
    .sort((a, b) => {
...
function printJson({ overall, perProject, perSubagent, perSkill }) {
  const out = {
    ...
    recent_prompts: recentPrompts(RECENT_PROMPTS),
    by_day: buildByDay(),
  }
...
// recent_prompts: recency-ranked, compact form. Includes ALL human prompts (no
// apiCalls filter — short prompts that resulted in no API calls still carry
// intent signal). This is the primary intent signal for goal-audit. The
// token-weighted topPrompts() variant was removed because it silently buried
// recent short prompts behind heavy historical planning prompts, defeating the
// purpose of an intent audit.
function recentPrompts(n) {
...
function printText({ overall, perProject, perSubagent, perSkill }) {
  ...
  // (Text-mode "MOST EXPENSIVE PROMPTS" section removed — goal-audit always
  // consumes --json output, and recent_prompts is the primary intent signal.)
