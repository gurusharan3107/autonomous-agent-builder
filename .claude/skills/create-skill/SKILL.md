---
name: create-skill
description: "Single entry for authoring, auditing, and optimizing project-local skills under `.claude/skills/<name>/`. Three lanes: Create (scaffold folder + SKILL.md template), Audit (deterministic checks via `scripts/audit.py` over frontmatter, naming, body size, description quality, ref integrity), Optimize (targeted fixes for audit findings — description rewrite, body→references/ split, frontmatter repair, name normalization). On invocation, ALWAYS asks which lane via AskUserQuestion unless the typed prompt names one. Use when the operator says 'create a skill for X', 'scaffold a skill', 'add a SKILL.md', 'audit a skill', 'check this skill', 'review my skills', 'validate skill conformance', 'optimize a skill', 'fix a skill description', 'shrink a skill', 'why isn't this skill activating', or any variant pairing skill / SKILL.md / .claude/skills with create / scaffold / audit / check / validate / optimize / shrink / fix / repair."
