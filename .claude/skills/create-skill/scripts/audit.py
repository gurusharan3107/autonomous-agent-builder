...
def audit_skill(skill_path: Path, strict: bool = False) -> list[dict]:
        ...
        findings.append(
            {
                "id": cid,
                "label": label,
                "severity": severity if not strict else "hard",
                "passed": passed,
                "detail": detail,
            }
        )
    ...
    finding(
        "A00",
        "skill is project-local (.claude/skills/)",
        "hard",
        not is_global,
    ...
    if name_match:
        ...
        finding(
            ...
            "hard",
            fname == skill_path.name,
            f"frontmatter={fname!r} dir={skill_path.name!r}",
        )
    ...
    finding(
        "A05", "description ≤1024 chars", "soft", len(desc_text) <= 1024, f"{len(desc_text)} chars"
    )

    # A06 — body ≤500 lines
    finding("A06", "SKILL.md body ≤500 lines", "soft", len(lines) <= 500, f"{len(lines)} lines")

    # A07 — Hard Rules
    finding("A07", "Hard Rules section present", "soft", bool(re.search(r"## Hard [Rr]ules", text)))

    # A08 — Workflow
    finding("A08", "Workflow section present", "soft", bool(re.search(r"## Workflow", text)))
    ...
    # A10 — evals
    finding(
        "A10", "evals/evals.json present", "soft", (skill_path / "evals" / "evals.json").exists()
    )
    ...
    finding(
        "A11", "description starts imperative", "soft", imperative, f"starts with {first_word!r}"
    )
    ...
    if ref_dir.exists():
        ...
        if ref_files:
            conditions_ok = all(f"reference/{rf.name}" in text for rf in ref_files)
            finding(
                "A12",
                "reference/ files have conditions in body",
                "soft",
                conditions_ok,
