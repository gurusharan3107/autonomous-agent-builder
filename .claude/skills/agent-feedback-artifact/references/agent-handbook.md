...
| Check | How to verify |
|---|---|
| Toggle survives 1-minute page auto-refresh | Devpulse template has a 60 s `setTimeout(location.reload, 60000)` — wait it out, widget should reappear automatically. |
| Trash removes locally AND on refresh | Click trash on a marker. Hard-reload. Marker should stay gone. |
| Clear-all removes everything AND on refresh | Click clear icon. Hard-reload. Zero markers. |
| Configure button opens to Agent tab | Click configure in the popover footer. The "Agent" tab should be the active one, not "UI". |
| Send → reply visible in status bar without manual refresh | Send a comment, wait. Agent's reply text should appear in the top-right status bar within ~5 s (pollStatus interval) or instantly (SSE push). |
| Auto-reload doesn't infinite-loop on multi-reload markers | Mark two markers `--reload` in quick succession. Exactly one page-state refresh should occur. |
...
1. Open DevTools on the page. Console: any CSP errors? → MV3 inline-script lesson above.
2. `chrome://extensions` → check the Hermes extension is loaded, not errored. Errors there mean manifest is invalid.
3. From the page console: `chrome.runtime` should be `undefined` (page world) but `document.getElementById('hermes-feedback-mount')` should be a node.
4. If mount node exists but `window.__agentFeedbackLoaded === false` → script didn't run → CSP issue → check the runtime is served via `<script src>` not inline.

### "Markers POSTed but agent never wakes"

1. `curl http://localhost:4177/api/feedback/status?artifact=/` — does the marker exist server-side? If no → widget POST failed (likely CORS or WSL2 IPv6 issue).
2. `ps -ef | grep agent-feedback-watch` — is the watch script running? If not → no wake source.
3. `ls /tmp/agent-feedback-<slug>/data/feedback-queue.json` — does the file the watch is watching exist?
4. `node scripts/agent-feedback-watch.mjs --root <serve-root>` directly in a shell — does it emit lines when you POST a marker?
...
1. Check the queue: did `mark.mjs` actually write `reload: true` into the item? Cat the queue file or `curl /api/feedback/status?artifact=/` and look for `reloadMode`.
2. Open DevTools → Network → filter `events` → is there an open EventSource connection? If not → widget isn't getting pushes; check SSE endpoint reachability.
3. In page console: `localStorage.getItem('agent-feedback:/:reloaded')` — does it contain the marker id? If yes → the wrap already saw and processed it (the page may have been reloaded already and you missed it). If no → wrap isn't being called.
4. If wrap isn't being called → check if SSE delivers `queue_changed` events (curl the events endpoint with a separate terminal while POSTing).
5. If SSE delivers but wrap isn't called → check the pollStatus wrap ordering bug (see lesson above).
...
Likely CORS / network. From DevTools Network panel:
...
If preflight fails → server isn't sending CORS headers (check server.mjs is current).
If POST returns 4xx → check payload shape (see `comments: []` requirement).
If both fine but widget shows "Send failed" → widget's `__FB_BASE` resolution; check `data-queue-origin` on the mount node matches a reachable URL.
