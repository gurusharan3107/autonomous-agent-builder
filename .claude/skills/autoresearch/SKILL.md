...
14. **Deterministic auto-apply vs model-backed escalation — the boundary is "does the fix introduce new substrate divergence?"** Auto-applied (catalogued, $0 retry): missing deps, pytest plugins, untracked file rm, DB row wipe. Escalated (`applied=False` + `proposed_questions`): tracked forbidden files (rm would commit→pollute history), git history pollution (re-snapshot decision), unknown error signatures (no validated remediation). When self_heal returns `proposed_questions`, `baseline.py` emits a `SELF_HEAL_ESCALATION <json>` line AND writes `<evidence-root>/SELF_HEAL_ESCALATION.json`. **After every baseline.py invocation (success OR abort), the calling agent MUST run `python3 .claude/skills/autoresearch/scripts/extract_escalation.py <evidence-root>` and check the exit code. Exit 1 means an escalation is present — invoke `AskUserQuestion` with the script's `askUserQuestion_args` payload BEFORE writing any chat reply. Exit 0 means proceed normally. Don't paraphrase the escalation into prose; the structured question IS the reply.** False fixes are worse than no fix.
...
17. **Lean discipline — every part earns its keep (drive the idiot index toward 1).** The loop's raw material is tiny: run a fixture, read `noncached_plus_output_tokens`, compare to baseline ± noise. Everything else is markup. **Apply the algorithm in order:** (1) question every requirement — name the person it serves and the decision it changes; if neither, it doesn't exist; (2) delete before you optimize — optimizing a part that shouldn't exist is the most common error; (3) only then simplify, accelerate, automate. If you're not adding ~10% back, you didn't cut enough. **A lane that produces no kept win in ≤3 cheap iterations is a candidate for kill-or-repurpose, not more scaffolding** — the loop already surfaced more value as a reliability fuzzer (P18/P23) than as a token optimizer (0 kept wins). Before adding ANY script / doc / gate / fixture, check it against [`references/lean-audit.md`](references/lean-audit.md) — the per-item idiot-index verdicts and the `docs/autoresearch/` retention boundary.
