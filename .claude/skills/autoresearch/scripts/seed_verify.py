...
def _safe_git(seed_dir: pathlib.Path, args: list[str], timeout: int = 30) -> tuple[bool, str]:
...
def verify_git(seed_dir: pathlib.Path, rules: dict) -> list[dict]:
    ...
    if max_commits is not None:
        ...
        if not ok:
            out.append({"clause": "git.max_total_commits", "status": "warn", "detail": stdout})
        else:
            try:
...
def verify_deps(seed_dir: pathlib.Path, rules: dict) -> list[dict]:
    ...
    if rules.get("pytest_must_collect"):
        ...
        if not tests_dir.exists():
            out.append(
                {"clause": "deps.pytest_must_collect", "status": "skip", "detail": "no tests/ dir"}
            )
        else:
            r = subprocess.run(
                [
...
def verify_db(seed_dir: pathlib.Path, rules: dict) -> list[dict]:
    ...
    try:
        ...
        return [{"clause": "db", "status": "warn", "detail": f"cannot open DB: {exc}"}]
    try:
        cur = conn.cursor()
        for t in tables:
            try:
...
def verify_seed(seed_dir: pathlib.Path, manifest: dict) -> dict:
    ...
    overall = (
        ...
        else ("warn" if any(f.get("status") == "warn" for f in findings) else "pass")
    )
    return {
        "overall": overall,
        ...
        "findings": findings,
        "remediation_summary": [
            f for f in findings if f.get("status") == "fail" and f.get("remediation_hint")
        ],
    }
...
def main() -> int:
    ...
    p.add_argument(
        "--seed-dir", type=pathlib.Path, default=None, help="defaults to seed_dir from manifest"
    )
    ...
    if args.json:
        ...
        print(
            f"seed_verify: {glyph[report['overall']]} {report['overall'].upper()}  "
            f"seed={report['seed_dir']}"
        )
        for f in report["findings"]:
            print(
                f"  [{glyph.get(f.get('status', 'warn'), '?')}] {f['clause']}: "
                f"{f.get('detail', '')}"
            )
