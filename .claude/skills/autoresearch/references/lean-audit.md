...
|---|---|---|---|---|
| `diagnose_hang.py` + `hang_watchdog.py` + `KNOWN_PATTERNS.md` + `hang-detection.md` | ~2300 | **extreme** | "You built a 2,300-line autopsy kit because your runs hang. Why do they hang? Fix the hang. Delete the coroner." | Fix root-cause hangs; collapse to a WAL-idle timeout + abort. Delete the pattern catalog. |
...
| `HARNESS.md` | 619 | **high** | "The code is the spec. A 619-line doc describing the program is a second program to keep in sync." | Delete; point readers at `run.py`. |
