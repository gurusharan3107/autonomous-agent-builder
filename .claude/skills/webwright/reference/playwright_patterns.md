...
# Exploration: cap ARIA output to keep context lean.
# Full snapshots are only needed during self-verify of a specific CP.
ARIA_EXPLORE_LINES = 80
...
async def main():
    async with async_playwright() as playwright:
        ...
        # networkidle waits for zero in-flight requests for 500 ms; SPAs with
        # polling/SSE connections never satisfy it and will always timeout.
        await page.goto("<START_URL>", wait_until="domcontentloaded")

        # SPA settle: domcontentloaded fires before React/Vue data-fetches
        # complete. 2 s covers most apps; use 3–4 s if you still see
        # "Loading…" placeholders in the snapshot.
        ...
        # NEVER use page.accessibility.snapshot() — removed in Playwright ≥ v1.40.
        # Always use page.locator("body").aria_snapshot() instead.
        snapshot = await page.locator("body").aria_snapshot()
        print("ARIA (exploration excerpt):")
        print("\n".join(snapshot.splitlines()[:ARIA_EXPLORE_LINES]))
...
- **Never** use `wait_until="networkidle"` — use `"domcontentloaded"` + `asyncio.sleep(2)`.
- **Never** use `page.accessibility.snapshot()` — use `page.locator("body").aria_snapshot()`.
- Cap exploration ARIA output to `ARIA_EXPLORE_LINES = 80` lines. Only read
  the full snapshot during self-verify when a specific CP requires it.
