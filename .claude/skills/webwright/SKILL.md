...
7. **Closeout — introspect and update the skill.** After every completed run,
   write `WORKSPACE_DIR/introspection.md` then use it to patch the skill.
   This is a closed loop: work done → self-introspection → skill update.

   **Write `introspection.md` covering:**
   - *What went perfectly* — steps that required zero correction (no changes needed).
   - *Friction points* — every place you hit an error, had to debug, or iterated
     more than once. For each one record:
     - What happened (the symptom)
     - Root cause (why the skill or reference didn't prevent it)
     - Fix type: **UPDATE THE STEP** (encode the correct form once so future
       agents don't re-derive it) vs **ADD A GUARD** (explicit "never do X"
       callout to stop agents before the mistake).
   - A decision table: friction # | symptom | fix type | target file + section.

   **Then apply every fix in the decision table to the skill files:**
   - Corrections to patterns, API calls, or defaults → edit
     `reference/playwright_patterns.md` or `reference/workflow.md`.
   - New "never / always" rules → add to the Hard Rules section of `SKILL.md`.
   - Never leave a friction point in `introspection.md` without a corresponding
     skill edit — the loop is only closed when both files are updated.
   - Once all skill edits are applied, delete `WORKSPACE_DIR/introspection.md`.
     Its content is now encoded in the skill; keeping the file wastes future
     context loading it.
...
- When the target is a product/builder dashboard with an operator chat interface,
  use natural language prompts only — no internal IDs, database keys, or system
  knowledge. Prompt as an operator who has only read the visible UI.
