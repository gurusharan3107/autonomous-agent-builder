...
def main() -> int:
    ...
    hr_missing = [
        title for title in skill_hard_rules
    ...
    if not de_counts_match:
        if n_artifact_de != n_operate_de:
            print(
                f'  Count mismatch: operate.md={n_operate_de} vs artifact-data={n_artifact_de}'
            )
        if dead_row_count != n_operate_de:
            print(
                f'  Count mismatch: operate.md={n_operate_de} vs HTML-rows={dead_row_count}'
            )
