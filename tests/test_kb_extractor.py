...
def test_sparse_python_package_blocking_docs_pass_contract(tmp_path, monkeypatch):
    (tmp_path / "README.md").write_text(
        "# Markup Library\n\n"
        "A compact Python library with a pyproject manifest and tests but no runtime dependency list.\n",
        encoding="utf-8",
    )
    (tmp_path / "pyproject.toml").write_text(
        "[project]\nname='markup-library'\nversion='0.1.0'\n",
    ...
    extractor = KnowledgeExtractor(
        tmp_path,
        output,
        doc_slugs=["technology-stack", "dependencies"],
    )

    result = extractor.extract()

    assert result["errors"] == []
    technology_stack = (output / "technology-stack.md").read_text(encoding="utf-8")
    dependencies = (output / "dependencies.md").read_text(encoding="utf-8")
    assert "### Representative production packages\n\n- None detected" in technology_stack
    assert "Sparse repositories can legitimately declare no runtime packages" in dependencies
    assert "compare the dependency hash" in dependencies
