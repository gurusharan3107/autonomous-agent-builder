...
def test_embedded_server_uses_project_codex_otel_endpoint_when_claude_disabled(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("AAB_CLAUDE_OTEL_ENABLED", "0")
    monkeypatch.delenv("AAB_CLAUDE_OTEL_ENDPOINT", raising=False)
    ...
    assert _local_otlp_endpoint(tmp_path) == "http://127.0.0.1:4318/v1/logs"
