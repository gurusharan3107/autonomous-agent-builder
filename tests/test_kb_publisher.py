...
def test_update_document_rejects_parent_traversal_doc_id(tmp_path, monkeypatch):
    local_root = tmp_path / ".agent-builder" / "knowledge"
    escaped_path = tmp_path / ".agent-builder" / "outside.md"
    escaped_path.parent.mkdir(parents=True)
    escaped_path.write_text("# Outside\n", encoding="utf-8")
    monkeypatch.setenv("AAB_PROJECT_ROOT", str(tmp_path))
    monkeypatch.setenv("AAB_LOCAL_KB_ROOT", str(local_root))

    with pytest.raises(PublishError, match="parent traversal"):
        update_document(doc_id="../outside.md", scope="local", title="Updated Outside")

    assert escaped_path.read_text(encoding="utf-8") == "# Outside\n"


def test_publish_document_rejects_escaping_file_name(tmp_path, monkeypatch):
    local_root = tmp_path / ".agent-builder" / "knowledge"
    escaped_path = tmp_path / ".agent-builder" / "escape.md"
    monkeypatch.setenv("AAB_PROJECT_ROOT", str(tmp_path))
    monkeypatch.setenv("AAB_LOCAL_KB_ROOT", str(local_root))

    with pytest.raises(PublishError, match="parent traversal"):
        publish_document(
            title="Escaping KB Doc",
            body=_local_body(),
            doc_type="context",
            tags=["context"],
            scope="local",
...
def test_update_document_accepts_canonical_existing_doc_id(tmp_path, monkeypatch):
    local_root = tmp_path / ".agent-builder" / "knowledge"
    monkeypatch.setenv("AAB_PROJECT_ROOT", str(tmp_path))
    monkeypatch.setenv("AAB_LOCAL_KB_ROOT", str(local_root))

    created = publish_document(
        title="KB Publish Boundary",
        body=_local_body(),
        doc_type="context",
        tags=["context"],
        scope="local",
    ...
    updated = update_document(
        doc_id=created["id"],
        scope="local",
        title="Updated KB Publish Boundary",
    )

    assert updated["id"] == created["id"]
    assert updated["title"] == "Updated KB Publish Boundary"
