...
class TestQualityGatesPhase:
    ...
    async def test_imp042_retry_count_reset_on_gate_pass(self, orchestrator):
        ...
        assert task.status == TaskStatus.PR_CREATION
        assert task.retry_count == 0, "retry_count must be reset to 0 on gate pass (IMP-042)"
...
@pytest.mark.asyncio
async def test_sprint_merge_preserves_dirty_local_checkout_changes(orchestrator, tmp_path):
    repo = tmp_path / "repo"
    workspace = tmp_path / "workspace"
    repo.mkdir()
    subprocess.run(["git", "init", "-b", "main"], cwd=repo, check=True, capture_output=True)
    ...
    (repo / "playwright.config.js").write_text("base config\n", encoding="utf-8")
    subprocess.run(
    ...
    (workspace / "src.js").write_text("console.log('ship')\n", encoding="utf-8")
    subprocess.run(["git", "add", "src.js"], cwd=workspace, check=True, capture_output=True)
    subprocess.run(
        ["git", "commit", "-m", "feat: app slice"],
        cwd=workspace,
        check=True,
    ...
    (repo / "playwright.config.js").write_text("local verification override\n", encoding="utf-8")

    sprint = Sprint(id="sprint-1", branch="task/t1")
    ...
    assert status == ""
    assert (repo / "src.js").read_text(encoding="utf-8") == "console.log('ship')\n"
    assert (repo / "playwright.config.js").read_text(encoding="utf-8") == "base config\n"
    ...
    assert "builder: preserve local checkout changes after sprint integration" in stash_list
