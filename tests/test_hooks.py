...
class TestAuditLogToolUse:
    ...
    async def test_session_start_policy_is_non_blocking(self):
        result = await session_start_context_policy(
            {},
            None,
            {
                ...
                "hook_policy": "audit",
                "selected_subagents": ["repo-researcher"],
            },
        )

        assert result == {}

    async def test_subagent_stop_audit_is_non_blocking(self):
        result = await audit_subagent_stop(
            {"subagent_name": "repo-researcher", "result": '{"status":"complete"}'},
            "tu-1",
            {},
        )
    ...
    async def test_completion_evidence_guard_blocks_when_required_markers_missing(self):
        result = await enforce_completion_evidence(
            {"result": "completed without proof"},
            None,
            {"required_evidence": ["commands", "evidence"]},
        )

        assert result["decision"] == "block"
        assert "commands" in result["reason"]

    async def test_completion_evidence_guard_allows_default_runs(self):
        result = await enforce_completion_evidence({"result": ""}, None, {})
