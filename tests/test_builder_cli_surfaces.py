...
def _plain(text: str) -> str:
    """Strip ANSI style/color escapes so help-text asserts are color-env independent.

    typer/Rich emit bold/dim/color escapes when the runner forces a terminal
    (CI sets FORCE_COLOR), and NO_COLOR does not override FORCE_COLOR — the
    escapes interleave with the text and break literal substring matches.
    """
...
def test_builder_help_exposes_single_startup_owner():
    ...
    assert "builder start --port 9876 Start the local dashboard and API" in out
    assert "Builder-owned local server lifecycle" in out
...
def test_logs_help_includes_raw_mode():
    ...
    out = _plain(result.stdout)
    assert "--raw" in out
    assert "Emit exact stored event" in out
