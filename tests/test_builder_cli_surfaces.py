...
def test_logs_analyze_returns_prompt_level_observability(monkeypatch, tmp_path):
    ...
    assert payload["observability_coverage"]["otel"]["service_name"] == "autonomous-agent-builder"
    ...
    assert prompt["tools"][0]["tool_name"] == "Glob"
...
def test_logs_analyze_surfaces_task_dispatch_correlation(monkeypatch, tmp_path):
    ...
    result = runner.invoke(app, ["logs", "analyze", "--session", session_id, "--json"])
...
def test_logs_analyze_includes_runtime_aggregates(monkeypatch, tmp_path):
    ...
    result = runner.invoke(app, ["logs", "analyze", "--session", "sess-analyze", "--json"])
...
def test_logs_analyze_does_not_flag_tool_events_before_first_run(monkeypatch, tmp_path):
    ...
    result = runner.invoke(app, ["logs", "analyze", "--session", "sess-empty", "--json"])
...
def test_metrics_show_json_includes_summary_and_next_step(monkeypatch):
    ...
    assert payload["raw_evidence"]["command"] == "builder metrics show --json --full"
