...
async def test_run_claude_sdk_prompt_applies_onecli_env(monkeypatch):
    ...
    assert captured["options_kwargs"]["env"]["CLAUDE_CODE_OAUTH_TOKEN"] == "placeholder"
    assert captured["options_kwargs"]["env"]["HTTPS_PROXY"] == "http://x:aoc_token@localhost:10255"
...
async def test_run_claude_sdk_prompt_uses_empty_env_mapping(monkeypatch):
    ...
    async def fake_prepare_onecli_runtime_env():
        return OneCLIRuntimeEnv(active=False, env={})
