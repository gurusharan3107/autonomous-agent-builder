...
def test_deterministic_script_candidates_detect_repeated_verifier_and_tools():
    ...
    assert by_code["build_verify_script"]["owner_lane"] == "builder_source"
    assert by_code["build_verify_script"]["next_actor"] == "builder"
    assert by_code["command_sequence_wrapper"]["owner_lane"] == "managed_repo_environment"
    assert by_code["command_sequence_wrapper"]["next_actor"] == "optimization_agent"


def test_output_truncation_candidate_stays_builder_owned():
    optimization = {
        "avoidable_cost_flags": [
            {"flag": "large_command_output", "count": 2},
        ],
    }

    candidates = deterministic_script_candidates({}, optimization)
    by_code = {item["code"]: item for item in candidates}

    assert by_code["output_truncation_artifact"]["owner_lane"] == "builder_source"
    assert by_code["output_truncation_artifact"]["next_actor"] == "builder"
    assert "Do not hand" in by_code["output_truncation_artifact"]["handoff"]


def test_output_truncation_candidate_uses_active_large_output_flags():
    optimization = {
        "avoidable_cost_flags": [
            {"flag": "large_command_output", "count": 2},
        ],
    ...
    assert "output_truncation_artifact" not in {item["code"] for item in candidates}
...
def test_optimization_decision_honors_clean_active_metrics_recommendation():
    optimization = {
        ...
        "top_cost_drivers": [
            {
                "agent_name": "agent-chat",
                "raw_tokens": 1_500_000,
                "noncached_plus_output_tokens": 300_000,
    ...
    decision = optimization_decision_summary(
        "codex_sdk",
        aggregates={"tool_observability": {"tool_counts": []}},
        optimization=optimization,
    )

    assert decision["next_action"] == "maintain_current_flow"
    assert decision["target_area"] == "observability"
    assert decision["reason"] == "no active avoidable optimization driver"
