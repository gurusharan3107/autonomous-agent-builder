...
def test_library_repo_without_entrypoint_has_no_blocking_architecture_unresolved(tmp_path, monkeypatch):
    ...
    (tmp_path / "pyproject.toml").write_text(
        "[project]\nname='importable-library'\nversion='0.1.0'\n",
        encoding="utf-8",
    )
    (tmp_path / "src" / "importable").mkdir(parents=True)
    (tmp_path / "src" / "importable" / "__init__.py").write_text("VALUE = 1\n", encoding="utf-8")
    collection = tmp_path / ".agent-builder" / "knowledge" / "system-docs"
    monkeypatch.setenv("AAB_PROJECT_ROOT", str(tmp_path))
    monkeypatch.setenv("AAB_LOCAL_KB_ROOT", str(tmp_path / ".agent-builder" / "knowledge"))
    ...
    assert gate.passed is True
    assert gate.unresolved_item_counts["system-architecture"] == 0
    content = (collection / "system-architecture.md").read_text(encoding="utf-8")
    assert "architecture boundary is anchored by checked-in manifests" in content
