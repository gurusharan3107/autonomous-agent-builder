...
async def test_onboarding_start_persists_selected_runtime(monkeypatch, tmp_path):
    builder_source_env = tmp_path / "builder-source.env"
    monkeypatch.setenv("AAB_BUILDER_SOURCE_ENV", str(builder_source_env))
    ...
    env_text = builder_source_env.read_text(encoding="utf-8")
