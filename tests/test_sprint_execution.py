...
async def test_sprint_execution_artifacts_annotate_queued_tasks(test_db) -> None:
    ...
    async with factory() as db:
        ...
        for task in tasks:
            ...
            assert sprint_execution["recommended_model"] == "anthropic/claude-sonnet-4-6"
...
@pytest.mark.asyncio
async def test_verify_task_runs_agentic_feature_verifier_before_durable_tests(
    test_db,
    tmp_path,
) -> None:
    _, factory = test_db
    async with factory() as db:
        ...
        async def run_agent(*args, **kwargs):
            ...
            return RunResult(output_text='{"status":"pass","recommended_next_action":"ship"}')
        ...
        assert order == ["feature-verifier", "feature-acceptance-tests", "build-verifier"]
    ...
    async with factory() as db:
        ...
        orchestrator._post_ship_optimization_cli_probe = AsyncMock(
            return_value=[
                ...
                {"command": "builder logs analyze --json", "result": "pass"},
            ]
        )
        orchestrator._run_agent = AsyncMock()
...
def test_build_verifier_failure_detects_markdown_failures() -> None:
    ...
    failure = orchestrator._build_verifier_failure("`npm run test` — **FAIL** (1/41)")
