...
def test_runtime_guidance_migrates_builder_claude_md_to_agents_md(monkeypatch, tmp_path) -> None:
    _write_builder_source_env(monkeypatch, tmp_path, 'RUNTIME_SDK="codex_sdk"\n')
...
def test_telemetry_env_template_enables_safe_local_defaults() -> None:
    ...
    assert 'RUNTIME_PROVIDER="claude_agent_sdk"' in text
    assert "AAB_CLAUDE_OTEL_DETAILED_BETA_TRACING=1" in text
    assert "builder.runtime=claude_agent_sdk" in text
    assert "AAB_CLAUDE_OTEL_LOG_RAW_API_BODIES=0" in text
    assert "AAB_CODEX_RUNTIME_TELEMETRY_ENABLED=0" in text
    assert "AAB_CODEX_TELEMETRY_SOURCE=codex_runtime_events" in text
    assert "AAB_CLAUDE_OTEL_LOG_TOOL_CONTENT=0" in text
...
def test_telemetry_env_status_distinguishes_unreachable_local_collector(
    ...
    source_env = _write_builder_source_env(
        monkeypatch,
        tmp_path,
        "\n".join(
            [
    ...
    assert status["path"] == str(source_env)
