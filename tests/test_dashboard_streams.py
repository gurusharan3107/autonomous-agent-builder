...
class TestDashboardStreams:
    async def test_board_stream_pushes_updated_snapshot(self, client, test_db, monkeypatch):
        ...
        task_id, gate_id = await _create_review_gate(client, test_db)
        _, session_factory = test_db
    ...
    async def test_approval_stream_reconnects_with_current_snapshot(
