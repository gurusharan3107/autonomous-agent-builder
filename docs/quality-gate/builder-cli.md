...
- `builder logs analyze --json` remains the prompt/session review lane and
  includes selected runtime, runtime-native telemetry health,
  builder-product telemetry health, and deterministic recommendation evidence
- `builder metrics show --json` remains the metrics decision lane and includes
  optimization summary, optimization decision, runtime decision summary, and
  deterministic script candidates when run evidence exists
