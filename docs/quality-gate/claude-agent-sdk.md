...
expectations:
  ...
  - "Claude SDK optimization preserves the model-backed agent loop for judgment work and tunes max turns, budget, effort, tool scope, permissions, hooks, subagents, tool search, context compaction, and telemetry instead of replacing intent with deterministic shortcuts"
  - "Claude SDK usage evidence separates input, output, cache creation, cache read, cost estimate, turn count, stop subtype, tool output pressure, and approval/question events"
...
- phase policy evidence records selected subagents, hook policy, permission
  ...
  reading prompt text
- Claude SDK evidence records per-result usage, cache creation/read tokens,
  result subtype, stop reason, `max_turns` / budget stops, and whether
  `AskUserQuestion` or approval callbacks paused the loop
- tool-search, subagent, and compaction choices are documented as SDK context
  controls rather than product semantics
...
- optimization changes make natural diagnosis or recommendation prompts
  deterministic just to save tokens
- raw prompts, tool arguments, tool output bodies, or raw API request/response
  bodies become default exported telemetry
