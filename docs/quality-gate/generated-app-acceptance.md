...
commands:
  ...
  - "Use Browser Use against the generated app localhost URL"
expectations:
  ...
  - "Browser Use proof records the visible URL, path, controls, and post-reload state; an app-local script may validate that artifact"
...
related_docs:
  - "docs/workflows/forward-engineering-autonomous-lifecycle-validation.md"
...
- adding Browser Use automation for generated-app validation

## Pass Signals

- the app starts from the disposable generated repo
- Browser Use reaches the feature through visible navigation
- primary user actions work through visible controls
- expected state survives navigation or reload
- a compact Browser Use proof artifact is available when the browser proof lane
  uses the in-app browser instead of local Chrome headless automation
...
- a failing or crashing local headless browser script is treated as equivalent
  to a passing Browser Use proof
- the sprint is marked shipped without generated-app Browser Use acceptance
