...
| Last Update | 2026-05-30 — M1.1 **IMP-020 closed** (chat lane denies ungranted mutating built-ins Edit/Write/Bash/MultiEdit/NotebookEdit and routes to capture→dispatch instead of offering an Approve/Deny card; design call resolved per CLAUDE.md dashboard-first doctrine; `agent_tool_policy.chat_mutating_builtin_denial` + `_authorize_chat_tool`; 43/43 affected tests green, ruff clean). **IMP-021 opened** (pre-existing env-sensitive doc-intent test fail surfaced during closeout — `canonical_ref` resolves `master` not `main` in temp repos). Branch `imp-018-015-019-browser-verify` (still local — IMP-018/015/019/020 ticked, IMP-019 live-verifier run still pending). *(Autoresearch M3.5 remains the in-flight focus.)* |
