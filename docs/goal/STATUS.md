...
| Current Item In Flight | **IMP-043(a) shipped (stabilization tick)** — guarded the dispatch failure-path FAILED persist with a fresh-session fallback (no more re-dispatch loop on an unrecoverable session); 45 `test_orchestrator.py` green, ruff clean, `state-integrity` gate ok. Branch `stabilize/imp-043`. Split IMP-043(b) → **IMP-044** (runtime.run no-timeout design decision, open). Also this session: PR #7 (efficiency-verdict-rework feature, `T:browser:pending`). Next stabilization targets: IMP-040 (await-future timeout), IMP-042 (retry-budget reset-on-progress). |
...
| Last Update | 2026-06-18 — Stabilization tick: **IMP-043(a)** durable FAILED-state persist on the dispatch failure path (fresh-session fallback; kills the re-dispatch loop). Also opened PR #7 (efficiency-verdict-rework: rework-share verdict + gate-timeout reprovisioning, `T:browser:pending`) + extended the stabilization loop's DISCOVER to read the open-IMP backlog. Prior: 2026-06-17 IMP-023 Fix B (`logs analyze` cost/run_count). |
