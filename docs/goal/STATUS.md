...
- **2026-05-23** — **Skill polish: preflight severity, KNOWN_PATTERNS catalog, lane docs, diagnose_hang matchers — 4 gaps closed before B–E re-baseline.** Audit before committing ~3–4h of compute: **(1)** `preflight.py` warn→fail for unstable fixtures on Recipes 2/3; added `--allow-unstable-promotion` opt-in override. **(2)** P15 (composite=0), P16 (CV>50%), P17 (seed dep gap) added to `KNOWN_PATTERNS.md`. **(3)** `iterate.md` clarifies `not_measured` is treated identically to `unstable` for the σ-floor requirement. **(4)** `diagnose_hang.py` gained matchers for P11/P14 (chained vs direct 409 disambiguated via builder_stdout_stderr.log), P15/P16/P17 (substrate-state matchers that read repo-level TSV + summary regardless of dump). Next session inherits today's hard-won context as deterministic checks.
- **2026-05-23** — **Seed drift: `pytest-asyncio` added to devpulse `requirements.txt`; fixture B 0/4 `feature_correct` was a seed defect, not Builder bug.** Fixture B's "notes feature with persistence" prompt drives the code-gen agent to write `async def test_*` (FastAPI + `httpx.AsyncClient`), but the seed's `requirements.txt` was missing `pytest-asyncio` — only declared under `[project.optional-dependencies] dev`. Builder's code-gen agent installed it ad-hoc in its task workspace (its own pytest passed all dots); the harness's clean post-FF venv didn't get it → async tests never executed → exit != 0 → `feature_correct=False`. Fix: added `pytest-asyncio>=0.23.0` to upstream `~/Builder-Workspace/devpulse/requirements.txt` + re-captured seed (sha256 `a9986867…` → `20af53c0…`). Bare-seed pytest went 107 → 139 passing — plugin was silently blocking 32 async tests in the seed itself. Truncated all 5 pre-fix fixture-B rows + per-prompt rows (measured substrate defect, not Builder). Documented in `baseline_variance.md § Seed drift`. Fixture A data stays (A doesn't generate async tests). **Next action**: re-baseline B–E with `baseline.py --fixtures B,C,D,E --n 5` to fill the σ-floor gap for promotion.
- **2026-05-23** — **iterations.html: progress chart.** Operator: "can we have a graph showing the current point, the baseline, with each iteration the graph gets updated?" Existing sparkline was 64px and only rendered when iterations existed (hidden at baseline-only state — useless until first iteration). Replaced with a 240px progress chart that always renders when baseline data exists. X-axis: combined sequence (baseline B1..N first, then iterations I1..M appended right). Y-axis: composite. Reference layers: shaded μ ± 2σ noise band, solid μ line, dashed 2σ-floor line ("win" threshold), vertical separator between baseline and iteration regions. Dots colored by status: baseline ships green, incomplete muted, crash red, iterations colored by verdict. Reserves 5 iteration slots even when none exist so the chart has visible headroom. Same SVG/no-framework approach as the prior sparkline.
- **2026-05-23** — **P16: composite formula switched to `noncached_plus_output_tokens` only.** Operator: "fix the composite formula." Diagnosis: `noncached_plus_output_tokens × operator_turns × wallclock_seconds` multiplied three correlated dimensions (a longer fixture run produces more of each); product compounded variance instead of averaging it, yielding CV=77.5% and 2σ-floor=-3.19e9 (negative — any candidate beats noise, gate useless). Methodology error, not a code bug. Fix: drop the multiplicative factors — `turns` and `wallclock` aren't billed and measure conversation length not efficiency; fixture is held constant across runs, so `noncached_plus_output_tokens` is the right cost comparison. Backfilled 9 existing baseline rows in `baseline_runs.tsv` from the same column (no re-runs). New fixture-A stats: μ=216,497, σ=31,831, CV=14.7% (green), 2σ-floor=152,835 (positive, gateable). Documented in 6 places: `run.py`, `OPTIMIZE.md`, `METRICS.md`, `README.md val_bpb`, `baseline.py` docstring, `iterations.html` methodology. Substrate variance was real but the metric was inflating it 5x.
