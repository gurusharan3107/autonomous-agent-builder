...
| Last Update | 2026-06-17 — IMP-023 Fix B: `logs analyze` headline now folds the session `agent_runs` cost aggregate into `total_cost_usd` + adds a `run_count` dispatch field (the M2.3 "enabler" — accurate cost surface before optimizing). Prior: 2026-06-16 restored the green `ruff check .` CI floor (lint-only; `ruff format` deferred). 034b frontend + 035b/c + live-verify still pending. |
