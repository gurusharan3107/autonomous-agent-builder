...
| Last Update | 2026-05-23 — M1.3 re-close in progress (1/7 done). `logs.py` 1679→1346 via `logs_runtime_aggregates.py` (408) + `logs_db_utils.py` (37); 61/61 `test_builder_cli_surfaces.py` green; baseline ratcheted; freshness sweep updated for new file location. 6 violations remaining. Next: `sprint_execution.py`, then the other 5. |
...
- **2026-05-23** — M1.3 re-close started: `cli/commands/logs.py` extraction landed (1679→1346 = -333). Two sibling modules: `logs_runtime_aggregates.py` (408 lines, public `runtime_aggregates(db_path, session_id)` + `selected_runtime_sdk()`, plus the supporting `_optimization_summary` / `_stop_reason_counts` / `_tool_counts` / `_approval_wait_summary` / `_provider_limit_summary` / `_phase_ceremony_summary` / `_repeated_retrieval_signal` helpers) and `logs_db_utils.py` (37 lines — `table_exists` / `table_columns` / `row_dict` / `maybe_json_dict`). `_selected_runtime_from_coverage` preserved at logs.py module level (test imports it). Bug side-effect: removed dead duplicate `_table_columns` (the second def silently shadowed the first safer one). `complexity-baseline.json` ratcheted 1679→1346 — this is the new floor; lint now reports `baseline_not_ratcheted_down` when files shrink past their baseline, so every extraction must update the baseline in the same commit. `freshness_sweep.py:check_logs_emits_session_scoped` updated to look at `logs_runtime_aggregates.py`. 61/61 `test_builder_cli_surfaces.py` green; sweep clean. M1.3 [ ] entry restructured with per-file sub-checkboxes; this commit ticks the `logs.py` sub-box per "commit only on ROADMAP tick". 6 violations remaining.
