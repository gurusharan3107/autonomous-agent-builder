...
| Last Update | 2026-05-23 — Fix lane closed 3 gate failures blocking N=5 baseline (`gate_pass_rate=0.5` in cycle 11): (1) `chunk_pressure_risk_false` — `local_fallback.py` called `metrics_json(session)` with wrong arg type; fixed by adding `load_metrics_response` standalone to `dashboard_api.py` (mirrors `load_board_response`). (2) `gate_pass_rate_full` — evidence command `builder backlog task list` tried workspace-stored port 9999 after server kill; switched to `builder board show --json` (has local DB fallback) + updated `evaluate_hard_gates` for board-section schema. (3) `feature_correct` — pytest ran Playwright acceptance tests requiring a live devpulse server; added `--ignore-glob=*playwright*`. Regression test: `test_load_local_metrics_uses_standalone_loader`. 76/76 affected tests green. Freshness sweep clean. |
