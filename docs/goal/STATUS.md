...
| Current Item In Flight | **IMP-045 shipped (Loop-4 outcome attribution)** — read-time `improved/regressed/flat/insufficient_data/not_measurable` verdict on applied optimization recommendations (token delta across the change's governed era; bounded-era window, no new table/FK/migration/cron/CLI). Closes the lifecycle's dormant `observed` bucket → `applied → measured`. 53 targeted + 196 downstream tests green, ruff clean, `agent-quality` gate ok. Branch `feat/loop4-outcome-attribution`. Prior: **IMP-043(a)** durable FAILED-state persist on dispatch failure path (branch `stabilize/imp-043`); IMP-043(b) → **IMP-044** (runtime.run no-timeout, open); PR #7 (efficiency-verdict-rework, `T:browser:pending`). Next stabilization targets: IMP-040 (await-future timeout), IMP-042 (retry-budget reset-on-progress). |
...
| Last Update | 2026-06-18 — **IMP-045** Loop-4 outcome attribution: applied recommendations now carry a read-time measured verdict (token delta across their governed era), turning the dormant `observed` lifecycle bucket into `applied → measured`. Read-time only, no new persistence. Prior same day: **IMP-043(a)** durable FAILED-state persist on the dispatch failure path; PR #7 (efficiency-verdict-rework, `T:browser:pending`). |
...
- **2026-06-18** — **Stabilization tick (Epoch 1, bug-fix before features) + loop improvement.** Fixed **IMP-043(a)**: `dispatch`'s exception handler persisted FAILED via an unguarded `self.db.flush()` after a *suppressed* rollback — on an unrecoverable session the flush raised, escaped `dispatch`, FAILED never committed, task re-dispatched in a loop. Fix guards the flush + falls back to a fresh `get_session_factory()` session (re-fetch by id + commit), the established side-channel pattern; new paired test fails-without/passes-with; 45 `test_orchestrator.py` green, ruff clean, `state-integrity` gate ok. Split **IMP-043(b)** (`runtime.run()` no-timeout) → **IMP-044** and left it open as a design decision (Codex runtime already has internal timeouts; orchestrator-level wrap is intent-dependent — not guess-fixed, per the Shared fix contract auto-fix gate). **Loop improvement (operator-invited, validated):** the codebase-review loop was *filing* findings (IMP-040..043) faster than anything *fixed* them — stabilization's DISCOVER only read TESTING.md + the current diff, never the open-IMP backlog. Extended stabilization's DISCOVER (`loops.json`) to also pull open `P1/P2` `T:backend:pending` ROADMAP IMPs (esp. codebase-review-filed); validated this tick by it surfacing IMP-043 as real, in-scope work. Branch `stabilize/imp-043`. Detail: CHANGELOG 2026-06-18 + ROADMAP M1.1 IMP-043/044.
- **2026-06-17** — **IMP-023 Fix B (closed): accurate cost surface — the M2.3 "enabler".** Chose the enabler slice of the cost/efficiency work first (get measurement right before optimizing). `builder logs analyze` headline reported `total_cost_usd=0` for chat-target sessions despite real sub-agent spend (Fix A had already fixed the token total). Fix: `_analyze_timeline` (`cli/commands/logs.py`) now falls back to the session-scoped `runtime_aggregates["totals"]["cost_usd"]` aggregate, mirroring the token fallback; added a distinct `run_count` headline field (`totals.runs`) for the dispatch fan-out. **Design call: did NOT overload `prompt_count`** — it stays `len(prompts)` (operator-chat-turn count, Bar 1 contract); a separate `run_count` surfaces dispatches. Evidence: headline cost/count read from `optimization_summary` does NOT feed `recommended_next_change` (that's token-based, already Fix-A-covered), so Fix B's value is operator cost visibility + autoresearch cost attribution, not flipping the optimizer verdict. New prove-fail test `..._imp023b`; analyze 9 + consumers 45 + `test_builder_cli_surfaces.py` 58 green; ruff clean. Detail: CHANGELOG 2026-06-17 + ROADMAP M1.1 IMP-023.
- **2026-06-16** — **Stabilization tick: CI ruff-lint floor restored.** A `/start` + stabilization sweep found `ruff check .` (the `ci.yml` py311 gate my memory calls "the only functional enforcement floor") had been **red since it was wired 2026-06-04** — 6/6 `ci.yml` runs failed at the Ruff-lint step, which **skips** the format-check and pytest steps, so the test wall never ran in CI for ~12 days. The prior "ruff clean" claims were scoped to changed files, not `ruff check .`. Fixed at root: py311 f-string-backslash **parse failure** (`install_hermes_chrome_bridge.py:117`, 8 invalid-syntax errors) + 103 autofixable + 56 manual (E402 reorders / SIM105→`contextlib.suppress` / targeted `# noqa` / B904 / B905 / E741 / SIM103). The F401 autofix stripped 2 test-surface re-exports from `routes/agent.py` (non-redundant `name as _name` alias, no noqa) → 2 suite failures → restored with `# noqa: F401` before commit. **Scoped lint-only by operator decision**: `ruff format` (216 files) deferred as a separate reviewed pass (the documented avalanche; [[feedback_ci_autofix_design]]). Evidence: `ruff check .` clean; full suite 1644 passed. Detail: CHANGELOG 2026-06-16 + ROADMAP M1.3.
