...
- [ ] `P2` **IMP-043** — orchestrator failure-path + agent-run robustness (codebase-review tick 2): (a) `dispatch` post-rollback persist (`orchestrator.py:337-340`) does `set_task_status(FAILED)`+`flush()` UNGUARDED after a suppressed `rollback()` — if `self.db` is unrecoverable the FAILED status never persists and the task is re-dispatched in a loop (the IMP-010 defense's own persist is fragile); fix: persist FAILED via a fresh short-lived `get_session_factory()` session. (b) `await runtime.run()` (`agent_run_lifecycle.py:361`, reached via `_run_agent`) has no timeout → a hung runtime strands the task in a non-terminal phase with no FAILED/CAPABILITY_LIMIT transition (same await-no-timeout class as IMP-040). Confirm no deeper runtime timeout before fixing. `T:backend:pending` `T:browser:na`
