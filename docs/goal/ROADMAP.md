...
- [ ] `P1` **IMP-024** — `cache_ratio` >> 1 (15797): divides by non-cached input. Fix `cache_ratio = cached/(cached+input)` clamp 0–1; 4 call sites (`logs.py`, `services/codex_optimization.py`) + fixtures. `T:backend:pending` `T:browser:na`
