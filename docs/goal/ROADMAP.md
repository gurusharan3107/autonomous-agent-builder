...
- [x] **IMP-023** — cost/token analyze headline read 0 (last-write-wins clobber). `_merge_run_status_telemetry` sums additive fields; `test_timeline_analysis.py`. Fix A (token fallback, 2026-05-31) + **Fix B (2026-06-17): `total_cost_usd` falls back to the session-scoped `agent_runs` cost aggregate; new `run_count` headline field exposes sub-agent dispatch fan-out without overloading `prompt_count` (operator-turn contract). `test_..._cost_and_run_count_fall_back_..._imp023b`.** `T:backend` `T:browser`
...
- [x] `P2` **IMP-037** — chat agent kept attempting app edits → hit the expected IMP-020 Bash/Edit denial (wasted round-trips). Fix: front-loaded a concise no-direct-edit rule in the RUNTIME chat prompt `_general_chat_prompt()` (`agent_prompt_builders.py` — NOT `definitions.py`): states Edit/Write/Bash on app files are disabled by design and app changes go via `mcp__builder__task_dispatch`. `test_general_chat_prompt_front_loads_no_direct_edit_rule` (presence + front-loaded before the LOOKUP RULE). Behavioral confirmation (chat agent stops attempting edits) pending a chat-lane observation. `T:backend` `T:browser:na`
- [x] `P1` **IMP-038** — **REFUTED as a builder defect (code-verified, 2026-06-17).** Hypothesis: AskUserQuestion answer controls were disabled by a *concurrent* running session. Code trace disproves it: `AgentDecisionActions.tsx` gates option buttons (L86), the no-option Send (L68) and the custom Send (L117) **only** on `submittingEventId === item.id` (per-item, while that item's own answer is mid-submit) — there is NO global/`running`/other-session condition; `composerSendDisabled` (`AgentPage.tsx:1040`) is likewise correct. The build-driver's "disabled because another session ran" was speculation; what it actually hit was the legitimately-disabled empty-custom **Send** (L117, `!draft.customText.trim()`) and/or the **hermes cursor-overlay click-interception** (its own friction #2) — a *tooling/bridge* issue, not builder source. RESIDUAL → investigate cursor-overlay pointer-event capture in the **hermes-chrome** lane (not builder). No builder fix. Surfaced + closed by the build→maintain flywheel verify-before-defect step. `T:backend:na` `T:browser:na`
- [x] `P2` **IMP-039** — **WITHDRAWN (not a builder defect, 2026-06-17).** The flywheel built in the existing `habit-tracker` project only because the driver pointed at the already-running `:9876` instance (bound to ONE app) and used its Agent page — where staying in the current app is CORRECT behavior. The real gap was in the FLYWHEEL procedure: "start a NEW managed app" must PROVISION one (`builder init --no-input` in a fresh `~/Builder-Workspace/<name>` + `builder start` serving it), not drive the pre-existing instance. Fixed in the `builder-test` loop DRIVE step (loops.json), not builder source. `T:backend:na` `T:browser:na`
- [x] `P2` **IMP-040** — **DONE (stabilization, 2026-06-19)** — chat-tool/approval `await future` (`routes/agent.py:442` AskUserQuestion + `:634` approval card) had NO cancel path: an unanswered question/approval pinned the chat run's live runtime session indefinitely (`request.is_disconnected()` only checked in the SSE generator). **Operator design call: cancel-on-disconnect.** Fix: `ChatSessionHub.cancel_session_pending_answers(session_id)` + `has_active_subscribers(session_id)` (`embedded/server/chat_state.py`); `event_generator`'s `finally` (`routes/agent.py:~1154`) cancels the session's pending-answer futures **only when no SSE subscribers remain** (multi-tab safe), raising `CancelledError` at the awaits and releasing the runtime session. Paired tests: cancel-only-matching-session, has_active_subscribers lifecycle, disconnect-cancels, second-subscriber-survives (`test_embedded_server_app.py`). ruff clean; `architecture-boundary` + `state-integrity` gates ok. `T:backend` `T:browser:na`
- [x] `P3` **IMP-041** — **DONE (stabilization, 2026-06-24)** — `routes/agent.py` had 3 near-identical tool-denial emission blocks (feature_spec / kb_validate / mutating-builtin) building the same `permission_denied` `denial_content` + `tool_error` payload, appending the event, and publishing. Extracted one `_emit_tool_denial(state, *, tool_name, tool_input, message, hint, detail)` helper; each call site now passes its lane-specific `message`/`hint`/`detail`/input and returns `_permission_deny(...)` with its own reason. Behavior-preserving (`json.dumps(..., sort_keys=True)` ⇒ byte-identical content); net −27 LOC. No new test (pure refactor): 22 denial-path tests + 29 `test_embedded_server_app` green; ruff clean; `architecture-boundary` gate ok. `T:backend` `T:browser:na`
- [x] `P2` **IMP-042** — **DONE (stabilization, 2026-06-19)** — `task.retry_count` was reset only on RECOVERY (`task_recovery.py:353`), never on a successful gate pass / forward progress, so a task carried consumed gate-retries across distinct gate cycles → premature `quality_gate_cap_exceeded` BLOCKED (cap = 3×`max_retries`, `orchestrator.py:783`). **Operator design call: per-gate budget, reset on forward progress.** Fix: `quality_gate_runner.py` resets `task.retry_count = 0` on the gate PASS/WARN → `PR_CREATION` forward-progress branch (`:145`), so each gate cycle gets a fresh budget. Paired test `test_imp042_retry_count_reset_on_gate_pass` (seeds `retry_count=2`, asserts `0` + `PR_CREATION` after pass). ruff clean; `state-integrity` gate ok. `T:backend` `T:browser:na`. (Note: part (a), the dead 3× cap on code/env/timeout paths, is a benign observation — those paths already escalate at `max_retries` in `gate_feedback.py`; left as-is, no behavior change needed.)
- [x] `P2` **IMP-043** — **DONE (stabilization, 2026-06-18)** — `dispatch` post-rollback persist (`orchestrator.py:336-340`) did `set_task_status(FAILED)`+`flush()` UNGUARDED after a suppressed `rollback()` → if `self.db` is unrecoverable the `flush()` raised, the exception escaped `dispatch`, FAILED never persisted, and the task was re-dispatched in a loop (the IMP-010 defense's own persist was fragile). Fix: guard the `flush()`; on failure log `phase_error_persist_fallback` and persist FAILED durably via a fresh short-lived `get_session_factory()` session that re-fetches the task by id and commits (the established side-channel pattern from `agent_run_lifecycle.py:252` / `orchestrator.py:1304`); `phase_error_persist_failed` logged if the row is absent. New paired test `TestDispatchFailedPersistFallback.test_failed_state_persisted_via_fallback_when_primary_flush_raises` (fails-without/passes-with). 45 `test_orchestrator.py` green; ruff clean; `state-integrity` gate ok. `T:backend` `T:browser:na`. (Part (b), the `runtime.run()` no-timeout class, split to IMP-044.)
- [x] `P2` **IMP-044** — **DONE (stabilization, 2026-06-19)** — `await runtime.run()` (`agent_run_lifecycle.py:361`) had no timeout and the **Claude runtime path had NO internal idle timeout** (confirmed — unlike Codex's `_TURN_EVENT_IDLE_TIMEOUT_SECONDS`), so a hung SDK stranded the task in a non-terminal phase forever. **Operator design call: inactivity (idle) timeout, NOT wall-clock; on expiry → recoverable FAILED, NOT CAPABILITY_LIMIT.** Fix: `_STREAM_EVENT_IDLE_TIMEOUT_SECONDS = 120.0` + `idle_timeout_seconds` param on `AgentRunner.run_phase`/`_execute_query` (`agents/runner.py`) — when set, each `receive_response()` step is bounded by `asyncio.wait_for` (clock resets per message); on `TimeoutError` returns `RunResult.error` with **no** capability-limit stop_reason → routes to FAILED. **Armed ONLY on the orchestrated lane** (`claude_runtime.run` forwards the param; `agent_run_lifecycle.py:361` passes the constant) — NOT the chat lane, whose `can_use_tool` legitimately blocks on the operator inside the stream (that's covered by IMP-040's cancel-on-disconnect). `failure_diagnosis.is_runtime_idle_timeout` → self-describing `issue="runtime_idle_timeout"` for Board "Recover". Paired tests: idle-timeout-surfaces-recoverable-error (`hit_capability_limit is False`), idle-clock-resets-on-messages, no-timeout-when-param-None (chat lane preserved), + 2 diagnosis tests. ruff clean; `claude-agent-sdk` + `architecture-boundary` + `state-integrity` gates ok. `T:backend` `T:browser:na`
...
- [x] `P2` **IMP-046** — **DONE (dogfooding sprint 1, 2026-06-19)** — the Board's `blocked_reason` for `final_checkout_build_failed` leaked the **raw build-verify tool dump** to the operator (e.g. `npm run lint FAIL: 59:106931 error 'RTCPeerConnection' is not defined …` — a minified `node_modules` bundle line, meaningless to a notes-app customer) and the wall of text buried the Board's Recover button. Surfaced by the browser-verifier driving a real dashboard recovery. Fix: `summarize_build_failure_for_operator(output)` (`orchestrator/deterministic_verification.py`) names the failed command(s) + points to run evidence; wired into both block sites (`orchestrator/sprint_lifecycle.py`, `services/run_reconciliation.py`). **Preserves the `final_checkout_build_failed:` routing prefix** (task_recovery keys off it) and keeps the **raw output in `verification_evidence`** (remediation re-derives detail via `_enriched_final_checkout_failure`'s fresh BuildVerify run, so no degradation). Paired tests: 3 helper unit tests (names cmd, no `RTCPeerConnection`/column leak, multi-cmd, capped fallback) + extended reconciliation integration test (`blocked_reason` summarized, raw stays in evidence). 66 targeted green; ruff clean; `architecture-boundary` + `state-integrity` gates ok. (M2.4 no-internals-leakage.) `T:backend` `T:browser` (recovery validated live: blocked→done in ~2 min). Residual friction (deferred, frontend): Board Recover-button visibility + Blocked-column-last layout.
- [ ] `P3` **IMP-047** — **Agent chat thread shows no mid-sprint progress** (dogfooding sprint 2, 2026-06-19). After "Delivery has started", the conversation thread surfaces no planning/design/implementation phase events; the next visible signal is "COMPLETE". The Board shows phase progress, but an operator watching the *thread* gets no status — fine for a 1m sprint, but a longer multi-task sprint would look stalled. VERIFY whether this is by-design (Board owns task progress, thread owns conversation) before fixing; if a gap, surface bounded sprint-phase events into the thread. Frontend + observability. `T:backend:pending` `T:browser:pending`
- [x] `P2` **IMP-048** — **DONE (codebase-review: onboarding.py, 2026-06-24)** — `start_onboarding` (`onboarding.py`) deduped concurrent starts with `asyncio.Lock` checked via `lock.locked()`, but the lock was acquired only INSIDE the scheduled task (`async with lock`) → TOCTOU: two near-simultaneous `POST /api/onboarding/start` both passed the precheck before either task acquired the lock, so the second still ran the full pipeline a second time on the already-onboarded repo (re-clearing/reseeding operational state). Fix: replaced the lock with a synchronous in-flight marker set (`_pipeline_inflight`) — the marker is added with NO `await` between the membership check and `asyncio.create_task`, so a concurrent start atomically bails; the task clears it in `finally`. Removed the now-unused `_project_lock` helper. Paired regression test `test_onboarding_start_dedupes_concurrent_starts` (two `gather`-ed starts ⇒ `run_count == 1`). Also deleted 2 confirmed-dead functions in the same slice (0 refs across src/+tests/): `onboarding_mode` and `_forward_engineering_feature_specs` (~66 LOC, superseded by `_classify_onboarding_mode` / `feature-list.json` seeding). 20 onboarding tests green; ruff clean; `state-integrity` + `architecture-boundary` gates ok. `T:backend` `T:browser:na`
