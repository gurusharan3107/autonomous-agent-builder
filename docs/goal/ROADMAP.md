...
- [ ] **Re-close the 0-violation gate** (regression discovered 2026-05-23 during Baseline lane preflight). Current state: 7 violations — 5 baseline_growth from the M2.3 Fix-lane work (`cli/commands/logs.py` 1679→1749, `db/models.py` 676→679, `embedded/server/agent_sprint_planning.py` 500→502, `services/sprint_execution.py` 825→828, `tests/test_builder_cli_surfaces.py` 2589→2734) + 2 missing_baseline on prior-session autoresearch scripts (`.claude/skills/autoresearch/scripts/introspect.py` 806, `scripts/autoresearch/run.py` 636). **Operator decision (2026-05-23): path B — extract rather than ratchet.** Per-file extraction plans recorded in `docs/quality-gate/complexity-baseline.json` (extraction_plan field) — `logs.py`: "Move command-family helpers and output builders into focused CLI modules before adding commands or options"; `sprint_execution.py`: "Split cohesive responsibilities into named owner modules"; etc. Constraint: sequential single-agent extraction per `.memory/feedback_extraction_constraints.md`. **Blocks M3.5 D1 (N=5 baseline) until 0 violations.** Next session work.
