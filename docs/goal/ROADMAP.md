...
- [ ] `P3` **IMP-041** — `routes/agent.py` has 3 near-identical tool-denial emission blocks (~60 LOC: feature_spec / kb_validate / mutating-builtin denials at ~469-490, ~517-538, ~591-612) → extract one `_emit_tool_denial(state, tool_name, tool_input, error_dict)` helper. High-confidence dedup, bounded; deferred from the codebase-review tick to keep the correctness fix clean. `T:backend:pending` `T:browser:na`
