...
- [ ] `P1` **IMP-038** — AskUserQuestion answer controls (option buttons + Send) could not be operated via the dashboard UI while a *different* session was concurrently RUNNING → operator forced to `/api/agent/chat/respond`. **Root cause UNCONFIRMED** — `composerSendDisabled` (`AgentPage.tsx:1040`) looks correct (does not disable on `loading` when a question is pending); the option-button disable lives in a deeper subcomponent below `agent-native.tsx` and was not pinned; the build-driver also flagged a *possible hermes cursor-overlay click-interception* artifact (may be tooling, not builder). NEXT: deliberate concurrent-session repro (long task running + 2nd thread + pending question) to confirm before any fix — do NOT symptom-patch. Surfaced by the build→maintain flywheel, 2026-06-17. `T:backend:pending` `T:browser:pending`
