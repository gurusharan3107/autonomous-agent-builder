...
- [ ] `P2` **IMP-037** — chat agent keeps attempting app edits → hits expected IMP-020 Bash/Edit denial (not a defect; wasted round-trips). Front-load the no-direct-edit + `task_dispatch` rule in the chat prompt (`agents/definitions.py`). `T:backend:pending` `T:browser:na`
