...
- [ ] **Re-close the 0-violation gate** (regression discovered 2026-05-23 during Baseline lane preflight). Operator decision: path B — extract rather than ratchet baselines. Per-file extraction plans live in `docs/quality-gate/complexity-baseline.json` (`extraction_plan` field). Sequential single-agent extraction per `.memory/feedback_extraction_constraints.md`. **Blocks M3.5 D1 (N=5 baseline) until 0 violations.** Per-file progress:
    - [x] `cli/commands/logs.py` 1679→1346 via two sibling extractions: `logs_runtime_aggregates.py` (408 lines — SQL aggregate machinery: `runtime_aggregates`, stop-reason / tool-counts / approval-wait / provider-limit summaries) and `logs_db_utils.py` (37 lines — shared sqlite helpers: `table_exists`, `table_columns`, `row_dict`, `maybe_json_dict`). `_selected_runtime_from_coverage` preserved at module level (test-imported per `.memory/feedback_extraction_constraints.md`). Bug side-effect: removed dead duplicate `_table_columns` (two definitions of the same name; second silently shadowed the safer first). Baseline ratcheted down from 1679 → 1346 in `complexity-baseline.json`. `freshness_sweep.py` updated to look for `session_scoped` in the new location. *(2026-05-23)*
    - [ ] `services/sprint_execution.py` 825→828 (baseline_growth +3 from M2.3 Fix lane).
    - [ ] `db/models.py` 676→679 (baseline_growth +3).
    - [ ] `embedded/server/agent_sprint_planning.py` 500→502 (baseline_growth +2).
    - [ ] `tests/test_builder_cli_surfaces.py` 2589→2734 (baseline_growth +145).
    - [ ] `.claude/skills/autoresearch/scripts/introspect.py` 806 (missing_baseline; prior-session autoresearch script).
    - [ ] `scripts/autoresearch/run.py` 636 (missing_baseline; prior-session autoresearch script).
