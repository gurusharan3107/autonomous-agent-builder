...
- [ ] `P2` **IMP-040** — chat-tool/approval `await future` in `_authorize_chat_tool` (`routes/agent.py:~442`) + the approval card (`~634`) have NO timeout/cancel-on-disconnect: an unanswered question/approval pins the chat run's live runtime session (Claude/Codex) indefinitely (`request.is_disconnected()` is only checked in the SSE generator, not here). DESIGN DECISION: accept "blocked-on-operator forever" or add a timeout / cancel-on-disconnect so a disconnected operator releases the runtime session. Surfaced by the codebase-review loop (tick 1, agent.py); medium-confidence resource-leak. `T:backend:pending` `T:browser:na`
...
- [ ] `P2` **IMP-042** — orchestrator retry-budget semantics need a decision (intent-dependent): (a) `_phase_quality_gates` 3×`max_retries` cap (`orchestrator.py:746-768`) is effectively dead for code/env/timeout failures — `gate_feedback.py` escalates to CAPABILITY_LIMIT at `max_retries` on those paths (`:140/:160/:187`); only the no-check remediation-thrash paths (`:129/:182`) ever approach the 3× cap. (b) `task.retry_count` is reset only on RECOVERY (`task_recovery.py:353`), never on a successful gate pass / forward progress, so a task carries consumed retries across distinct gates → premature CAPABILITY_LIMIT/BLOCKED. Define intended budget (per-gate vs cumulative) + add a reset-on-progress. Surfaced by codebase-review tick 2 (orchestrator.py). `T:backend:pending` `T:browser:na`
...
- [ ] `P2` **IMP-044** — `await runtime.run()` (`agent_run_lifecycle.py:361`, reached via `_run_agent`) has no orchestrator-level timeout → a hung runtime strands the task in a non-terminal phase with no FAILED/CAPABILITY_LIMIT transition. **DESIGN DECISION (same await-no-timeout class as IMP-040, do not guess-fix):** the Codex runtime already enforces internal idle/response timeouts (`codex_app_server_runtime.py` `_REQUEST_RESPONSE_TIMEOUT_SECONDS` / `_TURN_EVENT_IDLE_TIMEOUT_SECONDS`); the Claude runtime path (`runtime/claude_runtime.py:69`) needs verification. Decide: orchestrator-level wall-clock timeout wrapper vs trust per-runtime internals; and the on-timeout transition (FAILED vs CAPABILITY_LIMIT) given provider-limit blocked-state interplay. Split from IMP-043(b) at the stabilization tick that fixed (a). `T:backend:pending` `T:browser:na`
