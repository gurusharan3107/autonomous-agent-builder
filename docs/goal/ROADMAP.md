...
- [ ] `P2` **IMP-039** — "Create a *new* app" instruction ran in the existing `habit-tracker` workspace instead of a new project/workspace ("New thread" creates a conversation thread, not a new project). PRODUCT DECISION needed: is this expected (instruction joins the current project context) or a gap (no operator path to start a NEW app/project from the Agent page)? If a gap, add a "new project" affordance + new-app intent routing. Surfaced by the build→maintain flywheel, 2026-06-17. `T:backend:pending` `T:browser:pending`
