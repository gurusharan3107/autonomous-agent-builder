# TESTING.md — Negative-Scenario Browser Testing Roadmap

Browser-first bug hunt against the **builder product** (lane: `claude`). Vehicle:
a fresh managed **Kanban-board** app driven through the dashboard via
`/hermes-chrome`. Goal: surface where the lifecycle *falters* under negative /
edge input, then fix root causes (not symptoms). App-vs-builder routing: builder
defects → ROADMAP/`IMP-*`; app defects → the app's own backlog.

Mirror of this file renders in [goal-overview.html](goal-overview.html) §
"Browser Testing" (generated; never hand-edit the HTML — run
`python3 .claude/skills/status/scripts/build_goal_overview.py`).

## How tracking works

One scenario per line under a `### S<n>` surface group. Each carries an `S:`
state token (the only source of truth for the HTML mirror):

| Token | Meaning | Badge |
| --- | --- | --- |
| `S:pending` | not yet started | ⬜ |
| `S:inflight` | **current testing effort** — being exercised now | 🔄 |
| `S:pass` | exercised in-browser, behaved correctly | ✓ |
| `S:fail` | bug found (links the `IMP-*` / finding) | ✗ |
| `S:blocked` | can't run yet (prereq / environment) | ⛔ |
...
Line shape (parser-stable):
`` - `SC-NN` **Title** — <negative condition>. Expect: <correct behavior> (<anchor>). `S:<state>` ``
