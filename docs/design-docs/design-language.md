...
- Keep sprint-level Plan, Design, Implementation, Verify, Blocked, and Shipped
  controls in the central
  board strip. Do not repeat phase chips inside each lane header.
- Use sidebars for details: sprint stages open the full sprint
  artifact, and clicking a task opens the task handoff, acceptance criteria,
  runtime, cost usage, and gate context.
...
Five fixed status lanes: In progress, Needs review, Queued, Shipped, Blocked.
The board header owns a compact dispatch control and centered sprint strip:

- The board defaults to the current sprint. Use a compact sprint selector for
  `Sprint N`, earlier shipped sprints, and `All sprints`; do not mix old sprint
  cards into the current sprint lanes by default.
- `Sprint N` followed by `Plan`, `Design`, `Implementation`, `Blocked`,
  `Verify`, and `Shipped`.
- `Plan` and `Design` are complete once the shared sprint artifacts exist.
- `Implementation` is active only while implementation/integration runtime work
  is in flight.
- `Verify` is active after all implementation tasks are done and sprint-level
  verification is running or pending evidence.
- `Shipped` is active only after sprint verification passes and the approved
  backlog feature is marked done.
- `Blocked` is active only when current sprint tasks are blocked or failed; it
  must not remain active because of stale blocked text from a previous run.
- Clicking any sprint stage opens a right-side detail drawer with the full
  underlying sprint artifact. Do not keep the full orchestration table visible
  on the board by default.

Task cards stay compact and scan-first:

- Show status, title, progress, cost usage, turns, runtime, and duration.
- For Codex subscription runs, show cost usage as `subscription`, not `$0.0000`
  pretending to be metered usage.
- Task cards open a right-side task drawer instead of expanding in place.
- Empty lanes use the same lane footprint with a compact empty state so column
  rhythm stays stable.
