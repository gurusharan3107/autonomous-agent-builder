...
### 0. Pre-Commit Checks (`scripts/pre_commit_checks.py`)

**Purpose:** Block commits when product code changes would leave repo owner docs stale.

**What it tests:**
- `builder lint --json` runs for every commit
- Product code changes require a matching repo-doc owner update under
  `docs/references/`, `docs/workflows/`, `docs/quality-gate/`, or another
  maintained `docs/` owner file
- Changed surfaces run bounded executable proof, such as memory CLI tests,
  builder CLI surface tests, runtime tests, dashboard projection tests, or doc
  contract checks

This hook intentionally does not run generated KB freshness checks. KB
freshness remains owned by `builder knowledge validate --json` during builder
knowledge and documentation-refresh lanes.
...
**What it tests:**
- `builder memory add` creates valid files and returns post-mutation evidence
- Files have proper template structure
- Memory mutations reindex routing, run lint, and prove retrieval before success
