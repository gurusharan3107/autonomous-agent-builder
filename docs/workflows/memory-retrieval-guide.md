...
| Situation | Query |
|-----------|-------|
| Task on any surface (general precedent check at task start) | `builder memory search "<surface keyword>" --tag <surface-tag> --limit 100` |
| Browser/UI verification (board, agent page, dashboard, feature acceptance) | `builder memory search "<surface>" --tag browser-testing --limit 100` |
| Writing or evaluating a fix | `builder memory search "<symptom>" --tag fix-discipline --limit 100` |
| About to add a guard or defensive check | `builder memory search "root cause" --tag root-cause --limit 100` |
| About to change runtime/SDK policy (model, effort, thinking, prompt) | `builder memory search "<topic>" --tag sdk --limit 100` |
| About to mutate an app workspace under `Workspace/` or `aab-workspaces/` | `builder memory search "workspace" --tag workspace-ownership --limit 100` |
| About to edit AGENTS.md or runtime contracts | `builder memory search "<surface>" --tag <surface> --limit 100` *and* the relevant `workflow quality-gate <surface>` |
| Repeated error / recurring failure | `builder memory search "<error keyword>" --tag <relevant-tag> --limit 100` |
| Test work | `builder memory search "test <topic>" --tag testing --limit 100` |
| Server/runtime startup or shutdown | `builder memory search "<runtime topic>" --tag runtime --limit 100` |

Broad unfiltered search (no `--tag`) is acceptable only at the very start of a task as a precedent sweep.
...
- Transient state — current sprint plan, task IDs, per-run costs, token counts. Those live in `builder backlog`, `builder board`, and `builder metrics show`.
...
| Found the only working interaction for a UI element / fix recipe (≥2 attempts) | `pattern` | the surface + technique tag (`testing`, `runtime`, `chrome`, `browser-testing`) |
...
| `chrome` | Chrome plugin interactions (the active browser-based testing tool) |
| `browser-control-legacy` | Historical browser-control tooling superseded by the Chrome plugin. Use only for archaeology, not for new patterns. |
...
- Per-run cost or token counts — lives in `builder metrics show`
