# Browser Dashboard Testing Workflow

## Overview
This workflow covers browser-driven dashboard testing for
`autonomous-agent-builder`. Browser-visible validation is required for
forward-engineering and reverse-engineering lifecycle runs; command-line checks
are evidence and diagnosis, not substitutes for the user path.

Use the browser surface that matches the job:

- Browser Use / in-app browser: preferred default for localhost dashboard
  testing when available, especially Agent-page, Backlog, Board, Inbox, and
  approval interactions.
- Computer Use: use when validating the visible desktop browser exactly as a
  human sees it, or when the other browser surfaces are unavailable.
- Chrome DevTools MCP: use when you need console, network, accessibility-tree,
  DOM, or screenshot evidence for frontend diagnosis.

The Chrome DevTools MCP steps below remain the precise debugging lane, not the
only valid browser-validation method.

## Prerequisites
- Dashboard application running on the selected port
- For Browser Use: Codex Browser plugin available with in-app browser access
- For Computer Use: visible browser session available on the desktop
- For Chrome DevTools MCP: Chrome DevTools MCP server configured and Chrome
...
### Pages to Test
1. **Board** - Current sprint selector, task pipeline, phase strip, and status
...
- [ ] Board defaults to the current sprint when multiple sprints exist
- [ ] Older shipped sprint tasks are reachable through the sprint selector but
      do not appear in current-sprint lanes by default
