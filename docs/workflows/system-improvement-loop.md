...
Use [agent-quality-tuning-loop.md](/Users/gurusharan/Documents/remote-claude/active/apps/autonomous-agent-builder/docs/workflows/agent-quality-tuning-loop.md)
instead when the primary question is not "what product defect or regression is
visible?" but "how should the builder's prompt, tool, model, subagent, or
context strategy be tuned for higher quality or better context efficiency?"
...
- Browser/UI verification: use `@Chrome` first for the operator-visible dashboard and generated-app testing; if Chrome is unavailable, use `$browser-use:browser`; if that is blocked and browser control is still needed, use `@Computer`; use CLI/logs/metrics only as diagnosis evidence
