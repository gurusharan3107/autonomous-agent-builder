...
- Worktree tasks are dispatched into sprint phases (PR creation, build verify) without gating.
- Uncommitted workspace changes are auto-committed before branch integration to prevent orphaned file loss.
- If a temp-backed task worktree disappears while its git branch still exists,
  Builder prunes stale worktree metadata and reattaches the existing task
  branch instead of recreating it from scratch.
- During task and sprint integration, builder-owned runtime guidance files such
  as `AGENTS.md`, `CLAUDE.md`, and `.claude/CLAUDE.md` are snapshotted,
  removed before checkout or merge operations that would otherwise be blocked
  by untracked generated-app files, and restored after integration.
- Sprint finalization must not declare shipped state until the sprint branch can
  fast-forward into the target checkout. When `main` has moved independently,
  Builder rebases the sprint branch onto `main` before the final fast-forward
  and only cleans the untracked paths Git explicitly reports as merge
  overwrites.
...
- For non-git repositories, use an explicit plain-directory workspace mode.
  Unsupported or unexpected worktree failures must fail closed instead of
  silently downgrading to a workaround.
...
If the repo is not git-backed, use the explicit plain-directory workspace mode
and record that mode in task state. If the repo is git-backed and worktree
creation fails, fail closed and mark the task blocked or failed with the
concrete workspace error; do not continue in a different path as a workaround.
