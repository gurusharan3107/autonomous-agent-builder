...
| [iterations.json](iterations.json) | Per-iteration verdict + composite delta data. Regenerated by `.claude/skills/autoresearch/scripts/render_iterations.py` at every Baseline + Iterate closeout. | Living | Harness output |
| [iterations.html](iterations.html) | Visual map: baseline + every iteration's verdict, 6-hard-gate status, A→E promotion. Reads `iterations.json` plus an embedded fallback block between `__ITERATIONS_DATA_START__` / `__ITERATIONS_DATA_END__` markers. Regenerated by `.claude/skills/autoresearch/scripts/render_iterations.py`. Do not hand-edit. | Living | Operator view |
