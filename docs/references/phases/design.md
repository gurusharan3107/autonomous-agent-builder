...
- one or more bounded task packets that can enter pre-provisioned workspaces at
  the `design -> implementation` boundary
- for sprint-generated work, compact task/file ownership hints and verification
  strategy derived from the shared sprint design
...
implementation continues. Low-risk local sprint tasks covered by an approved
sprint plan/design do not require separate per-task design approval unless
design changes locked scope, adds risk, adds external dependencies, or
materially increases sprint size.
