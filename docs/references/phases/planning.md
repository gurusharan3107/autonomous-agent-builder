...
- acceptance criteria
- for sprint-enabled forward delivery, a sprint-scope recommendation that
  defaults to one shippable feature unless direct dependencies require more
- next handoff into sprint planning, task generation, or delivery dispatch
...
Current repo mapping: `pending`, `queued`, `planning`, and the interactive
feature-backlog interview lane used for existing-product feature requests.
Sprint planning may create several generated tasks from one approved backlog
feature; the backlog feature remains the product outcome, not the task record.
