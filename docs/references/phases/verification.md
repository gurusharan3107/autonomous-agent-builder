...
satisfied should it create or update durable Playwright acceptance tests. On
later verification runs for the same feature, run the deterministic Playwright
feature tests first; if they are missing, stale, or failing, wake
`feature-verifier` to inspect whether the product or the test is wrong and then
rerun the deterministic acceptance check.

For forward-engineering generated apps, Browser Use is the preferred acceptance
proof for user-visible behavior. The proof should exercise visible navigation,
...
Browser Use proof artifact, but it must not hide a failed or missing browser
