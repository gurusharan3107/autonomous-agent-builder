...
|---|---|---|---|---|
| Agent | Send prompt | Starts or continues the selected/new thread; shows selected runtime and thread runtime separately. Typed operator prompts are interpreted by the selected model, which may inspect bounded Builder state and ask a structured question when intent is unclear. Deterministic zero-token behavior is reserved for explicit UI controls or system refreshes with fixed product semantics, not natural prompt interpretation. | Creates chat events and run status under selected runtime, or deterministic status events for explicit UI/system refreshes when no runtime work is needed | `user_message`, runtime-specific `run_status`, deterministic status `run_status`, tools/questions/errors |
...
| Agent | Answer question / approval | Resolves the pending event on the same thread/runtime. Question and approval controls stay inline in the Conversation timeline; visible option rows and approval buttons submit directly rather than opening a dialog or only updating local draft state. | Appends answer event and resumes run when applicable | `ask_user_question_answer` or `tool_approval_answer` |
...
Natural operator wording must keep the same continuation semantics across
runtimes. If the Board already has current-sprint dispatchable work, prompts
such as "continue the remaining verification task" or "continue building my
app" should dispatch that task in either runtime lane instead of falling back
to sprint planning.

Runtime-native user questions are still Builder dashboard UI. Whether the
question came from Claude `AskUserQuestion` or Codex `request_user_input`,
the Agent page must render plain operator wording and sanitize persisted cards
before display if a runtime supplies internal terms such as backlog, sprint,
task id, lifecycle, bounded, raw/full logs, chunk, or token pressure. The model
still decides when to ask; the dashboard owns the visible wording boundary.
Runtime-native assistant prose that asks for delivery permission is the same
class of operator decision. If a model says the work is ready to start and asks
whether to proceed or hold, the Agent page must render that as an inline
question/approval with readable controls instead of requiring the operator to
reply to plain text or understand internal lifecycle language.

Agent-page chat sessions allow one active runtime turn at a time. The dashboard
must reserve the run slot before persisting the next user message, so a rejected
concurrent request cannot create a transcript item that no Claude or Codex run
will answer.
...
6. Board task cards must prefer the latest model-backed agent run for runtime,
   agent, model, cost, token, turn, duration, and observability display when a
   later deterministic verifier/check run exists for the same task. The
   deterministic follow-up remains visible in run history, but it must not erase
   the attribution for the runtime that implemented the work.
7. Agent Run trace rows must preserve the runtime/provider that produced the
   historical run event. Runtime-specific timeline icons are attribution, not
   current-runtime controls: Codex SDK rows show Codex attribution, Claude Agent
   SDK rows show Claude attribution, and Samantha/Realtime Voice rows show
   OpenAI attribution.
8. Trace compaction may collapse adjacent uninformative tool-use rows into one
   counted timeline entry, but the raw/full trace lane must keep the underlying
   event detail available for diagnosis.
9. Quality-gate failures are review evidence, not proof that implementation is
   still actively running. A task with real quality-gate failure evidence should
   appear in the review lane for operator inspection, and the Board should not
   project it as pending or active only because the status is a verification
   phase.

## Board Action Rules

- The dispatch button is enabled only when the selected sprint has a
  dispatchable queued task and no visible task is currently running.
- While a task is running, the Board shows a disabled running state instead of
  offering a second dispatch action for the same sprint.
- Sprint-stage indicators derive from visible task state first when it is more
  specific than persisted summary status. Blocked, review, active, and queued
  stage hints must match the lanes the operator can see.
- When one persisted sprint row contains multiple approved feature slices, the
  Board must show separate visible sprint summaries per feature slice. Completed
  slices render as shipped sprint history; the current blocked or active slice
  remains the selected current sprint. Do not collapse all generated tasks into
  one `Sprint 1` card when the operator completed earlier sprint slices.
...
- Observability shows why the decision is valid: selected runtime,
  runtime-history rows, current native telemetry emission for the selected lane,
  config readiness for inactive native lanes, historical runtime access,
  Builder product telemetry, capability gaps, phase decisions, tool-event
  coverage, and deterministic recommendations.
...
- `builder logs analyze --session <id-or-prefix> --json` mirrors the dashboard decision fields with
...
- Verify Observability shows selected-lane current emission, inactive-lane
  config readiness, historical runtime access, and Builder product telemetry as
  separate areas, and never reports Builder product telemetry as
  `collector unknown`.
