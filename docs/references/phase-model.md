...
- `planning`
  ...
  Continuation handoff note: after a sprint plan creates dispatchable Board
  tasks, short operator prompts such as `start`, `continue`, or `go ahead` are
  phase-state continuation signals. The Agent page must dispatch the next
  Builder task instead of treating the prompt as generic model-backed chat,
  otherwise app edits, task state, Recent Agent Runs, and token telemetry can
  diverge.
  Dispatchability policy is shared across legacy `/api/dispatch` and embedded
  `/api/tasks/{id}/dispatch`: both routes must use the same task-status,
  backlog-status, duplicate-run, and provider-limit response contract.
- `design`
...
- `verification`
  ...
  sprint must prove the generated app through build/test/lint plus
  Chrome-visible acceptance evidence. A shell or app-local proof script may
  validate a recorded Chrome artifact when local headless Chrome is unstable.
- `integration`
