...
- preserve existing user-authored `CLAUDE.md`, `.claude/CLAUDE.md`, or
  `AGENTS.md`
- create or repair a root `.env` with runtime settings plus telemetry-safe
  Claude OTEL and Codex JSONL defaults; only the selected runtime lane is
  enabled
...
- `builder logs analyze --json` also mirrors the dashboard telemetry decision
...
- `builder agent sessions --json` should expose the repo-scoped chat session list with `sdk_session_id` when present so an agent can correlate builder sessions with selected-runtime continuity
...
- local read-only inspection mode should preserve the same repo-local semantics
  for session lookup and telemetry fields when the API is unavailable
...
Metrics payloads must include a compact `optimization_summary` when run
telemetry exists. The summary uses raw tokens as the primary score and exposes
non-cached + output tokens, cache ratio, phase ceremony tokens, avoidable-cost
flags, top cost drivers, benchmark status, and the next recommended optimization
change without requiring raw database inspection.
...
- compact `--json` output keeps `raw_evidence.command` pointing to
  `builder metrics show --json --full` for deeper run payloads
...
- when `builder logs` is sparse, the CLI should still provide product-owned
  secondary evidence via `builder metrics show --json --full` and
