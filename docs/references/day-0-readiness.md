...
builder-generated baseline between those filenames. Builder-owned runtime
selection, provider settings, Claude OAuth, Codex SDK settings, and telemetry
configuration live in the autonomous-builder source `.env`, not in generated app
`.env` files. Generated app workspaces may keep project-local non-secret
artifacts, such as Codex `.codex/config.toml`, so observability is available
from the first agent run without moving Builder configuration into the app.
...
- `builder init` creates or repairs runtime/provider/model settings, Claude
  OAuth/OTEL keys, and Codex runtime telemetry keys in the autonomous-builder
  source `.env`. The selected runtime lane is enabled and the inactive lane is
  disabled. Generated app `.env` files are not accepted as the owner for these
  Builder-owned keys.
...
- Day-0 readiness evaluates Builder runtime/auth/telemetry settings from the
  autonomous-builder source `.env`; it must not require or trust generated app
  `.env` files for `RUNTIME_*`, `AAB_*`, Claude OAuth, or Codex SDK settings.
