...
Playwright tests must be changed by builder-owned agents or deterministic
builder actions. The operator should see those updates as agent-run timeline
...
- `builder logs analyze --json` is the agent-facing bridge for prompt/session
...
unreachable collectors can trigger a telemetry-readiness recommendation, while
complete builder-product telemetry lets the agent trust Board, Metrics,
Observability, and `builder logs analyze` as product evidence.
