...
- Keep Claude runtime guidance self-contained and aligned with selected runtime
  behavior. Codex optimization instructions stay in `AGENTS.md`, which is not
  imported here.
...
- The orchestrator, not the user, owns follow-up work selection for explicit
  product events: after completion, approval, provider-limit reset, or a
  button/control action, builder may resume the preserved task or start the next
  ready task through product state. Typed operator prompts are different: the
  selected model must interpret the prompt, inspect bounded Builder state, choose
  tool calls, and ask through `AskUserQuestion` when the product ask is unclear.
- Agent intelligence is part of the product. Save tokens by giving agents
  bounded Builder tools, compact retrieval, and explicit evidence surfaces, not
  by replacing evidence-seeking requests with generic deterministic prose.
  Deterministic shortcuts are only for exact persisted-state reads or explicit
  UI controls with a fixed product contract; verification, diagnosis, prompt
  interpretation, recovery planning, and other judgment calls must use the
  tool-enabled Agent lane.
...
- When `RUNTIME_SDK=codex_sdk`, builder uses the local Codex app-server/SDK
  ...
  analysis.
- Treat recent-session evidence as lane-scoped. `codex_sdk` sessions prove
  Builder product trends, not Claude Agent SDK mechanics; use Claude runtime
  probes and logs before changing Claude-only policy.
...
- The optimization phase is model-backed. After build verification, the
  ...
  replaces.
- When `builder logs analyze` reports `maintain_current_flow`, keep runtime
  prompt structure stable and fix only the specific issue named by evidence.
...
- This source repo is not a generated app workspace. Run product session/log
  sweeps from initialized app workspaces; never initialize this repo just to get
  `.agent-builder` state.
...
- During forward/reverse lifecycle validation, do not use mutating `builder`
  ...
  behavior. Use `workflow --docs-dir docs summary autonomous-lifecycle-validation`.
...
- For recent-session tuning, use compact Builder CLI evidence first:
  `builder agent sessions --limit 100 --full --json`, `builder logs analyze
  --session <id> --json`, and `builder logs --error --compact --json`.
...
## Entry Points
- Browser-based dashboard testing: use the Chrome plugin for operator-visible validation.
- Dashboard-first lifecycle validation: `workflow --docs-dir docs summary autonomous-lifecycle-validation`
