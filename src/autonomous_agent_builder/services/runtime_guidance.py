...
def _active_runtime_sdk(project_root: Path) -> str:
    env = _parse_env_file(project_root.resolve() / ENV_FILE)
...
def ensure_project_telemetry_env(
    ...
    *,
    ...
    """Ensure repo-local .env has runtime-specific telemetry defaults."""
    root = project_root.resolve()
...
def render_project_telemetry_env(
    *,
    ...
    """Render safe, local-first runtime telemetry defaults."""
...
def render_project_runtime_guidance(
    *,
    ...
    resolved_mode = mode if mode in {"forward_engineering", "reverse_engineering"} else "forward_engineering"
    resolved_sdk = _normalize_sdk_value(sdk)
    command_values = _normalize_commands(commands)
    renderer = _render_codex_guidance if _runtime_guidance_kind(resolved_sdk) == "codex" else _render_claude_guidance
...
def runtime_guidance_contract_status(project_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "status": "generated_contract" if not missing_sections and command_slots else "generated_contract_incomplete",
        "sdk": sdk,
        "kind": kind,
        "expected_path": _runtime_guidance_root_filename(sdk),
        "path": str(path),
...
def telemetry_env_status(project_root: Path) -> dict[str, Any]:
    """Return readiness status for the selected runtime telemetry lane."""
    root = project_root.resolve()
    path = root / ENV_FILE
    if not path.exists():
        return {
    ...
    unsafe_keys = [
        key
        for key in _CONTENT_TELEMETRY_KEYS
    ...
    ok = active_enabled and inactive_disabled and collector_ok and not unsafe_keys
    if ok:
        status = "configured"
...
def _discover_commands(project_root: Path, *, language: str) -> dict[str, str]:
    ...
    if language == "python":
        ...
        if (project_root / "ruff.toml").exists() or "ruff" in _read_text(project_root / "pyproject.toml").lower():
...
def _discover_entrypoints(project_root: Path, scan_summary: dict[str, Any] | None = None) -> list[str]:
    scan = scan_summary or {}
    scanned = scan.get("entrypoints")
    if isinstance(scanned, list) and scanned:
        return [str(value) for value in scanned[:10]]
    matches: set[str] = set()
    for pattern in ("main.py", "app.py", "server.py", "manage.py", "src/**/main.py", "src/main.*", "cmd/**/main.go"):
