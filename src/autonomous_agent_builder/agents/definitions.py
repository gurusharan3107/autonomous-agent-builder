...
AGENT_DEFINITIONS: dict[str, AgentDefinition] = {
    ...
    "feature-verifier": AgentDefinition(
        ...
        prompt_template=(
            ...
            "Real-browser verification (preferred for user-facing web features) — exact procedure:\n"
            "1. `mcp__browser__resolve_app_url` to get the serve command + URL, then start that server "
            "in the background with log redirection (record the PID; never foreground it).\n"
            "2. `mcp__browser__navigate` to the URL ONCE. This opens a single dedicated, operator-visible "
            "browser tab and returns the landed page context; that one tab is reused for every later "
            "browser call and closed automatically at teardown — do not navigate repeatedly to 're-open' it.\n"
            "3. Read with `mcp__browser__page_context` first (compact ~1 KB: url/title/headings/buttons/inputs) "
            "to confirm the render, then `mcp__browser__read_text` to assert the ACTUAL rendered values/copy "
            "an acceptance criterion requires (e.g. a label, count, or formatted string is really on screen).\n"
            "4. Exercise behavior with `mcp__browser__click_text` (cursor-driven clicks) and `mcp__browser__fill` "
            "(form inputs); after each interaction re-check with one `page_context` or `read_text`. Confirm each "
            "acceptance criterion through real UI behavior, route changes, and reload/persistence — not jsdom alone.\n"
            "5. Capture one `mcp__browser__screenshot` as visual proof before you finish, and put its path plus "
            "the concrete URLs/values you confirmed into the `browser_evidence` field of your JSON result.\n"
            "6. If `mcp__browser__navigate` returns `bridge_unavailable`, fall back to the deterministic acceptance "
            "command and record that browser proof was unavailable (a weaker evidence tier).\n\n"
            "Required workflow:\n"
