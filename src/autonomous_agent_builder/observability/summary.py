    ...
    return {
        ...
        "available_signals": [
            ...
            for signal, enabled in (
                ...
                ("tool_events", not aggregates.get("tool_observability", {}).get("missing_tool_events")),
            )
            if enabled
        ],
        "counts": {
            "tools": int(aggregates.get("tool_observability", {}).get("agent_run_event_count") or 0),
            "errors": 0,
            "delegations": 0,
        },
        "otel": {
...
def _codex_coverage(
    ...
    if tool_state.get("missing_tool_events"):
        missing.append("tool_events")
    if "chunk_pressure_large_event" in flag_names or "large_command_output" in flag_names:
...
def _telemetry_health(
    ...
    codex_native_status = _native_status(
        ...
        missing=[] if codex_status.get("enabled") else [str(codex_status.get("reason") or "codex_otel_missing")],
    )
...
def _deterministic_recommendations(
    ...
    native = telemetry_health.get(native_key, {}) if isinstance(telemetry_health.get(native_key), dict) else {}
    collector_status = str(native.get("collector_status") or native.get("collector", {}).get("status") or "")
    if collector_status in {"missing", "configured_unreachable", "invalid_endpoint"}:
        recommendations.append(
            _deterministic_recommendation(
                ...
                evidence={"selected_runtime": selected, "collector_status": collector_status or "missing"},
            )
        )
    if aggregates.get("tool_observability", {}).get("missing_tool_events"):
        recommendations.append(
...
def _record_recommendation_decision(
    ...
    if existing and priority.get(str(existing.get("lifecycle_status")), 0) > priority.get(lifecycle_status, 0):
        return
    decision = {
        "code": code,
    ...
    for status_key in ("applied", "rejected", "not_applicable", "deferred", "observed"):
        lifecycle[status_key] = [
            item for item in lifecycle[status_key] if item.get("code") != code
        ]
...
def _recommendations(
    ...
    if runtime.get("selected_runtime_sdk") == "codex_sdk":
        if "codex_token_usage" in missing:
            recommendations.append(_recommendation("codex_token_usage_missing", "high", "Codex token usage missing", "Check app-server tokenUsage events before cost optimization."))
        if "model_effort_fields" in missing:
            recommendations.append(_recommendation("model_effort_missing", "medium", "Model or effort missing", "Persist model and effort on every Codex run."))
        if "tool_events" in missing:
            recommendations.append(_recommendation("tool_events_missing", "medium", "Tool events are missing", "Persist command/tool events before tuning routing."))
        if "chunk_pressure" in missing:
            recommendations.append(_recommendation("chunk_pressure", "medium", "Chunk pressure detected", "Trim large command output before reinjecting context."))
        top_driver = (optimization.get("top_cost_drivers") or [{}])[0]
        if top_driver and top_driver.get("agent_name") == "code-gen":
            recommendations.append(_recommendation("code_gen_raw_tokens", "medium", "Code-gen dominates raw tokens", "Tighten implementation briefs and file hints before lowering model effort."))
        for flag in optimization.get("avoidable_cost_flags") or []:
            ...
            if name in {"pr_lane_without_explicit_pr_target", "prompt_over_phase_budget"}:
                recommendations.append(_recommendation(name, "medium", name.replace("_", " "), "Remove avoidable model ceremony from low-risk local sprints."))
    else:
        if "otel_metrics_exporter" in missing or "otel_logs_exporter" in missing or "otel_traces_exporter" in missing:
            recommendations.append(_recommendation("claude_otel_missing", "high", "Claude OTEL exporters missing", "Configure Claude Agent SDK OTEL before relying on external telemetry."))
        if "otel_otlp_endpoint" in missing:
            recommendations.append(_recommendation("claude_otlp_placeholder", "medium", "Collector endpoint missing", "Use a real OTLP collector endpoint."))
        if coverage.get("otel", {}).get("sensitive_data_flags"):
            recommendations.append(_recommendation("sensitive_telemetry_flags", "medium", "Sensitive telemetry flags enabled", "Review content logging before exporting telemetry."))
        if "tool_events" in missing:
            recommendations.append(_recommendation("tool_events_missing", "medium", "Tool events are missing", "Persist tool events before tuning model routing."))
    ...
    if int(aggregates.get("provider_limits", {}).get("count") or 0) > 0:
        recommendations.append(_recommendation("provider_limits_present", "medium", "Provider limits present", "Resume ready blocked tasks before queueing new work."))
    open_rule_codes = {
        str(item.get("code") or "")
    ...
    if not recommendations and not open_rule_codes:
        recommendations.append(_recommendation("baseline_ready", "info", "Observability baseline is usable", "Continue collecting run evidence before changing routing policy."))
    return recommendations
    ...
    for payload in rows:
        ...
        if reset_at:
            ...
            if reset_at <= now:
                ...
                waiting += 1
    return {"available": True, "count": len(rows), "ready_to_resume": ready, "waiting_for_reset": waiting, "reset_at": reset_times}
...
def _repeated_retrieval_signal(tool_counts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    summary_rows = [
        row for row in tool_counts if str(row.get("tool_name") or "") in summary_tools
    ]
