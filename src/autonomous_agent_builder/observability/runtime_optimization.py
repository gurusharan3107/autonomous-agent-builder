...
def deterministic_script_candidates(
    ...
    flag_names = {
        str(item.get("flag") if isinstance(item, dict) else item)
        for item in flags
    }
...
def optimization_decision_summary(
    ...
    *,
    ...
    top_driver_name = str(top_driver.get("agent_name") or "") if isinstance(top_driver, dict) else ""
    top_driver_tokens = (
...
def _claude_capabilities() -> tuple[RuntimeCapability, ...]:
    return (
        RuntimeCapability("user_questions", "User questions", True, "AskUserQuestion top-level", "None", "", "Keep operator questions out of subagents."),
        RuntimeCapability("approvals", "Approvals", True, "permission callbacks and human review", "approval gates", "", "Persist approval state and resume the same run."),
        RuntimeCapability("model_effort", "Model and thinking", True, "model plus ThinkingConfig", "agent runtime policy", "", "Route thinking by phase risk."),
        RuntimeCapability("subagents", "Subagents", True, "SDK agents and .claude/agents", "main-lane execution", "", "Use isolated specialists only when context or policy changes."),
        RuntimeCapability("hooks", "Hooks and guardrails", True, "PreToolUse/PostToolUse/SubagentStart/SubagentStop", "builder guardrails", "", "Use hooks for deterministic enforcement and telemetry."),
        RuntimeCapability("telemetry", "Telemetry and cost", True, "ResultMessage plus OTEL", "builder estimates", "", "Prefer native cost, usage, model_usage, turns, and stop reason."),
        RuntimeCapability("tool_routing", "Tool routing", True, "tool permissions and MCP", "builder tool allowlist", "", "Keep safe discovery auto-approved."),
        RuntimeCapability("context", "Context management", True, "Claude compaction plus project memory", "builder compact briefs", "", "Use memory and compact handoffs before broad rereads."),
        RuntimeCapability("deterministic_work", "Deterministic work", False, "No single native script planner", "builder scripts and quality gates", "deterministic_script_candidates", "Promote repeated operations into scripts."),
        RuntimeCapability("workspace_recovery", "Workspace and recovery", True, "stable cwd and resume state", "builder workspaces", "", "Recover from persisted state before rerunning."),
    )


def _codex_capabilities() -> tuple[RuntimeCapability, ...]:
    return (
        RuntimeCapability("user_questions", "User questions", True, "request_user_input top-level", "approval cards", "", "Keep operator questions out of subagents."),
        RuntimeCapability("approvals", "Approvals", True, "granular approval policy and rules", "builder approval gates", "", "Use SDK policy for risky actions."),
        RuntimeCapability("model_effort", "Model and effort", True, "model, review_model, effort, model catalog", "agent runtime policy", "", "Route model and effort by phase risk."),
        RuntimeCapability("subagents", "Subagents", True, "Codex subagents with max threads/depth", "main-lane execution", "", "Use only for bounded sidecar work."),
        RuntimeCapability("hooks", "Hooks and guardrails", False, "No Claude-style subagent hook parity in current path", "Codex rules plus builder guardrails", "codex_hook_parity", "Expose the fallback instead of pretending parity."),
        RuntimeCapability("telemetry", "Telemetry and cost", True, "app-server events, token usage, rate-card credits", "builder estimates", "", "Track raw tokens, noncached+output, credits, and chunk pressure."),
        RuntimeCapability("tool_routing", "Tool routing", True, "tool_search and deferred tool loading", "builder tool allowlist", "", "Defer large tool schemas where possible."),
        RuntimeCapability("context", "Context management", True, "prompt caching, context window, auto-compaction", "builder compact briefs", "", "Keep stable prefixes and dynamic context late."),
        RuntimeCapability("deterministic_work", "Deterministic work", False, "No single native script planner", "builder scripts and quality gates", "deterministic_script_candidates", "Promote repeated operations into scripts."),
        RuntimeCapability("workspace_recovery", "Workspace and recovery", True, "workspace cwd and app-server run evidence", "builder workspaces", "", "Diagnose from events and state before rerun."),
    )
