...
def _is_placeholder_endpoint(value: str | None) -> bool:
    ...
    return bool(normalized) and any(marker in normalized for marker in _PLACEHOLDER_ENDPOINT_MARKERS)
...
def _repo_owned_env() -> dict[str, str]:
    ...
    env = {
        ...
        "OTEL_EXPORTER_OTLP_PROTOCOL": os.environ.get(
            "AAB_CLAUDE_OTEL_PROTOCOL", "http/protobuf"
        ),
        "OTEL_EXPORTER_OTLP_ENDPOINT": endpoint,
        "OTEL_METRIC_EXPORT_INTERVAL": os.environ.get(
            "AAB_CLAUDE_OTEL_METRIC_EXPORT_INTERVAL", "2000"
        ),
        "OTEL_LOGS_EXPORT_INTERVAL": os.environ.get(
            "AAB_CLAUDE_OTEL_LOGS_EXPORT_INTERVAL", "1000"
        ),
...
def resolve_claude_observability(extra_env: dict[str, str] | None = None) -> ClaudeObservabilityConfig:
    ...
    sensitive_flags = [key for key in _CONTENT_FLAGS if effective.get(key)]
