...
def _general_chat_prompt(
    ...
    *,
    ...
    prompt = (
        ...
        "This chat lane never edits the app directly: Edit/Write/Bash on app files are disabled "
        "by design, so do NOT attempt a direct edit (it only returns a denial and wastes a turn) — "
        "to change the app, dispatch a Board task with `mcp__builder__task_dispatch`. "
