...
def _local_knowledge_root(request: Request) -> Path:
    return _project_root(request) / ".agent-builder" / "knowledge"


def _resolve_kb_document(kb_root: Path, filename: str) -> Path | None:
...
async def get_kb_status(request: Request) -> KnowledgeBaseStatus:
    """Get current knowledge base status."""
    kb_root = _local_knowledge_root(request)
...
async def extract_knowledge(payload: ExtractionRequest, request: Request) -> ExtractionResponse:
    ...
    workspace_path = _project_root(request)
    kb_path = _local_knowledge_root(request) / DEFAULT_LOCAL_KB_COLLECTION

    try:
        from autonomous_agent_builder.knowledge import KnowledgeExtractor

        log.info("starting_deterministic_extraction", doc_types=payload.doc_types)

        extractor = KnowledgeExtractor(
            workspace_path=workspace_path,
            output_path=kb_path,
            doc_slugs=payload.doc_types,
        )
        ...
        if payload.run_validation:
            ...
            gate_result = await gate.validate_async(model=payload.model)
...
async def validate_knowledge(request: Request) -> ValidationResponse:
...
async def get_document(filename: str, request: Request) -> dict[str, Any]:
    """Get a specific knowledge base document."""
    kb_root = _local_knowledge_root(request)
    doc_file = _resolve_kb_document(kb_root, filename)

    if doc_file is None or not doc_file.exists() or not doc_file.is_file():
...
async def delete_knowledge_base(request: Request) -> dict[str, str]:
