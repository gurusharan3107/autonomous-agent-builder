...
from autonomous_agent_builder.knowledge.publisher import PublishError, parse_markdown_document
...
def extract_tags(content: str) -> list[str]:
    ...
    try:
        return parse_markdown_document(content).tags
    except PublishError:
        pass
    ...
    for index, line in enumerate(lines[1:], 1):
        ...
        inline_tags = [
            tag.strip().strip("\"'") for tag in tag_content.split(",") if tag.strip().strip("\"'")
        ]
        if inline_tags:
            return inline_tags
...
def extract_frontmatter_field(content: str, field: str) -> str | None:
    ...
    try:
        ...
        value = parsed.extra_fields.get(field)
        if value is None:
            return None
        return str(value)
    except PublishError:
...
def extract_frontmatter_int(content: str, field: str) -> int | None:
    """Extract an integer field from YAML frontmatter when present."""
    if field == "version":
        try:
            return parse_markdown_document(content).version
        except PublishError:
            pass
...
def extract_title(content: str, fallback: str) -> str:
    """Extract a document title through the canonical markdown parser."""
    try:
        return parse_markdown_document(content).title
    except PublishError:
        pass
...
def find_backlinks(doc_id: str, doc_title: str, kb_path: Path) -> list[dict]:
    ...
    for file_path in kb_path.rglob("*.md"):
        ...
        # Skip self
        if relative_path.as_posix() == doc_id.replace("\\", "/"):
            continue
...
def find_similar_docs(doc: dict, kb_path: Path, limit: int = 10) -> list[dict]:
    ...
    for file_path in kb_path.rglob("*.md"):
        ...
        if relative_path.as_posix() == doc["id"].replace("\\", "/"):
            continue
...
def _get_kb_path(scope: KBScope, request: Request) -> Path:
    """Get the knowledge base directory path based on scope."""
    if scope == "global":
        return Path.home() / ".claude" / "knowledge"
    return _project_root(request) / ".agent-builder" / "knowledge"


def _resolve_kb_document(kb_path: Path, doc_id: str) -> Path | None:
    resolved = (kb_path / doc_id).resolve()
    if not resolved.is_relative_to(kb_path.resolve()):
        return None
    return resolved
...
def _list_kb_files(kb_path: Path, doc_type: str | None = None) -> list[dict]:
    ...
    for file_path in kb_path.rglob("*.md"):
        ...
        try:
            ...
            source_title = extract_frontmatter_field(content, "source_title")
            source_url = extract_frontmatter_field(content, "source_url")
            card_summary = extract_frontmatter_field(content, "card_summary")
            detail_summary = extract_frontmatter_field(content, "detail_summary")
            version = extract_frontmatter_int(content, "version") or 1
            ...
            docs.append(
                {
                    "id": file_path.relative_to(kb_path).as_posix(),
                    "task_id": "",
...
async def list_kb_docs(
    ...
    kb_path = _get_kb_path(scope, request)
    ...
    if tags:
        ...
        docs = [doc for doc in docs if selected_tags.issubset(set(doc.get("tags", [])))]
...
async def get_all_tags(
    ...
    if tags:
        selected_tags = set(t.strip() for t in tags.split(","))
        docs = [doc for doc in all_docs if selected_tags.issubset(set(doc.get("tags", [])))]
    else:
        docs = all_docs
    ...
    if tags:
        ...
        for tag in tag_counts:
            if tag in selected_tags:
                ...
                has_docs = any(test_tags.issubset(set(doc.get("tags", []))) for doc in all_docs)
                if has_docs:
                    available_tags.add(tag)
    else:
...
async def get_related_docs(
    ...
    kb_path = _get_kb_path(scope, request)
    file_path = _resolve_kb_document(kb_path, doc_id)
    ...
    try:
        ...
        for link in wikilink_ids:
            ...
            for candidate in kb_path.rglob("*.md"):
                ...
                if (
                    link.lower() in relative_path.as_posix().lower()
                    or link.lower() in candidate.stem.lower()
                ):
                    try:
                        ...
                        wikilink_docs.append(
                            {
                                "id": relative_path.as_posix(),
                                "title": extract_title(link_content, candidate.stem),
        ...
        # Find similar docs
        doc_data = {"id": doc_id, "tags": extract_tags(content)}
        similar = find_similar_docs(doc_data, kb_path, limit=5)

        return {"wikilinks": wikilink_docs, "backlinks": backlinks, "similar": similar}
...
async def get_kb_doc(
    ...
    if file_path is None or not file_path.exists() or not file_path.is_file():
        raise HTTPException(status_code=404, detail=f"KB document not found: {doc_id}")
