    ...
    for index, task_id in enumerate(generated_ids):
        ...
        items.append(
            CurrentSprintItemSummary(
                id=task_id,
                title=title,
                status=(task_statuses or {}).get(task_id, "pending"),
...
async def _build_sprint_summary(
    ...
    *,
    ...
    generated_ids = [str(task_id) for task_id in (sprint.generated_task_ids or [])]
    sprint_tasks = visible_tasks or [task for task in all_tasks if task.id in set(generated_ids)]
    active_phase = _display_sprint_phase(sprint, sprint_tasks)
    parent_sprint_tasks = [task for task in all_tasks if task.id in set(generated_ids)]
    ...
    return CurrentSprintSummary(
        ...
        generated_tasks=_generated_sprint_tasks_from_plan(
            ...
            task_statuses={str(task.id): _status_str(task) for task in sprint_tasks},
        ),
...
async def load_board_response(db: AsyncSession, project_root: Path | None = None) -> BoardResponse:
    ...
    all_tasks = [
        ...
        and backlog_item_reaches_board(
            task.feature.status.value
            if hasattr(task.feature.status, "value")
            else str(task.feature.status)
        )
    ]
