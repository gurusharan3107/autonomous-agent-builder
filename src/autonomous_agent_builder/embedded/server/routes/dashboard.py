...
def _serialize_run(run: AgentRun) -> MetricsRunItem:
    ...
    return MetricsRunItem(
        ...
        observability=getattr(run, "observability", None),
        started_at=run.started_at,
        completed_at=run.completed_at,
    )


def _serialize_task_run(run: AgentRun) -> TaskAgentRunSummary:
    cost = estimate_run_cost(
        model=getattr(run, "model", "") or "",
        input_tokens=run.tokens_input,
        cached_input_tokens=run.tokens_cached,
...
async def _load_metrics_response(db: AsyncSession) -> MetricsResponse:
    ...
    total_estimated_cost_usd = sum(r.estimated_cost_usd for r in all_runs)
    credit_values = [r.estimated_codex_credits for r in all_runs if r.estimated_codex_credits is not None]
    total_estimated_codex_credits = sum(credit_values) if credit_values else None
    runtime_decision_summary_payload: dict[str, Any] = {}
    optimization_decision_payload: dict[str, Any] = {}
    deterministic_candidates: list[dict[str, Any]] = []
    try:
        db_path = find_agent_builder_dir(Path.cwd()).resolve() / "agent_builder.db"
        observability = dashboard_observability_summary(db_path)
        runtime_decision_summary_payload = observability.get("runtime_decision_summary", {})
        optimization_decision_payload = observability.get("optimization_decision", {})
        deterministic_candidates = observability.get("deterministic_script_candidates", [])
    except Exception:
    ...
    return MetricsResponse(
        ...
        total_estimated_codex_credits=total_estimated_codex_credits,
...
async def features_json(request: Request, db: AsyncSession = Depends(get_db)):
    """Feature list from the builder-managed DB backlog."""
    ...
    return FeatureListResponse(
        project_name=payload.get("project_name") or project_root.name,
        total=payload.get("total", 0),
        done=payload.get("done", 0),
        pending=payload.get("pending", 0),
        features=[_feature_list_item_from_dict(f) for f in payload.get("features", [])],
    )
...
def _build_task_item(t: Task) -> TaskBoardItem:
    ...
    return TaskBoardItem(
        ...
        dependencies=[
            str(item)
            for item in (t.feature.dependencies if t.feature else [])
            if str(item).strip()
        ],
        sprint_execution=_task_sprint_execution(t),
        agent_name=latest_run.agent_name if latest_run else "",
        runtime_sdk=getattr(latest_run, "runtime_sdk", "") if latest_run else "",
        provider=getattr(latest_run, "provider", "") if latest_run else "",
        model=getattr(latest_run, "model", "") if latest_run else "",
        effort=getattr(latest_run, "effort", None) if latest_run else None,
        cost_usd=latest_run.cost_usd if latest_run else 0,
        total_cost=total_cost,
        tokens_input=latest_run.tokens_input if latest_run else 0,
        tokens_output=latest_run.tokens_output if latest_run else 0,
        tokens_cached=latest_run.tokens_cached if latest_run else 0,
        ...
        observability=getattr(latest_run, "observability", None) if latest_run else None,
        agent_runs=[
            _serialize_task_run(run)
            for run in sorted(list(t.agent_runs or []), key=lambda item: _sortable_timestamp(item.started_at))
        ],
        activity_timeline=[
            TaskActivityEvent(**item)
            for item in build_task_activity_timeline(
                list(t.agent_runs or []),
                blocked_reason=t.blocked_reason or "",
            )
        ],
...
def _normalize_runtime_model_details(value: Any, fallback_model: str) -> Any:
    if isinstance(value, dict):
        return {key: _normalize_runtime_model_details(item, fallback_model) for key, item in value.items()}
...
def _compact_sprint_plan_summary(
    ...
    *,
    ...
    return SprintPlanSummary(
        ...
        runtime_tool_strategy=runtime_strategy,
        ...
        design_details=design_details if isinstance(design_details, dict) else {},
    )
...
def _generated_sprint_tasks_from_plan(
    ...
    for index, task_id in enumerate(generated_ids):
        ...
        spec = task_specs[index] if index < len(task_specs) else {}
        title = str(
        ...
        dependencies = [
            str(value).strip()
            for value in (
                batch.get("depends_on_batches")
                or spec.get("depends_on_batches")
                or []
            )
            if str(value).strip()
        ]
        runtime_strategy = batch.get("runtime_tool_strategy") or plan.get("runtime_tool_strategy") or {}
        sprint_execution = {
            ...
            "recommended_model": _display_runtime_model(
                batch.get("recommended_model") or spec.get("recommended_model") or plan.get("planning_model"),
                _selected_runtime_model(),
            ),
            ...
            "context_strategy": str(batch.get("context_strategy") or plan.get("context_strategy") or ""),
            "runtime_tool_strategy": runtime_strategy if isinstance(runtime_strategy, dict) else {},
            "implementation_brief": str(
                spec.get("implementation_brief")
                or batch.get("implementation_brief")
                or ""
            ),
            "file_ownership_hint": str(
                spec.get("file_ownership_hint")
        ...
        items.append(
            CurrentSprintItemSummary(
                ...
                title=title,
                status="done",
                description=str(spec.get("purpose") or sprint_execution["implementation_brief"] or ""),
...
def _display_sprint_phase(sprint: Sprint, tasks: list[Task]) -> str:
    ...
    if verification_status in {"pass", "passed", "shipped"}:
        return "shipped"
    if generated_count and done_count == generated_count:
        return "verify"
    if status_values & _BLOCKED:
        return "blocked"
    phase = sprint.phase.value if hasattr(sprint.phase, "value") else str(sprint.phase or "")
    if phase in {"scope", "planning"}:
        return "planning"
    if phase == "queued":
        return "implementation"
    if phase in {"design", "implementation", "verify", "shipped", "blocked"}:
        return phase
    return "planning"


def _phase_statuses(active_phase: str) -> dict[str, str]:
    display_order = ["plan", "design", "implementation", "blocked", "verify", "shipped"]
    phase_to_stage = {
        "scope": "plan",
...
async def _build_sprint_summary(
    ...
    sprint_tasks = [task for task in all_tasks if task.id in set(generated_ids)]
    ...
    return CurrentSprintSummary(
        sprint_id=sprint.id,
        label=sprint.label or "Sprint 1",
        ...
        generated_tasks=_generated_sprint_tasks_from_plan(generated_ids, plan),
        verification_status=sprint.verification_status or "pending",
...
async def load_board_response(db: AsyncSession, project_root: Path | None = None) -> BoardResponse:
    ...
    if project_root is not None and await sync_forward_engineering_feature_backlog(db, project_root):
        await db.commit()
    ...
    all_tasks = [
        ...
        if task.feature
        and (not project_id or task.feature.project_id == project_id)
        and backlog_item_reaches_board(
            task.feature.status.value if hasattr(task.feature.status, "value") else str(task.feature.status)
        )
    ]
...
async def observability_json():
    """Runtime observability evidence as JSON — consumed by React frontend."""
    try:
        db_path = find_agent_builder_dir(Path.cwd()).resolve() / "agent_builder.db"
    except ProjectNotFoundError as exc:
        raise HTTPException(status_code=404, detail=exc.hint or "Builder project not found") from exc
...
async def _load_command_index_response(db: AsyncSession) -> CommandIndexResponse:
    items: list[CommandPaletteItem] = [
        ...
        CommandPaletteItem(id="route-observability", kind="route", label="Observability", description="Open SDK diagnostic evidence", route="/observability"),
