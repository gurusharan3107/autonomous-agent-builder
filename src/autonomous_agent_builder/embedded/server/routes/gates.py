...
async def submit_approval(
    ...
    if task:
        if decision == ApprovalDecision.APPROVE:
            ...
            reason = data.get("reason") or data.get("comment", "") or "PR changes requested"
            ...
            task.blocked_reason = data.get("reason") or data.get("comment") or "Approval rejected"
    ...
    if task and should_dispatch:
        ...
        if task_status not in _NON_DISPATCHABLE_STATUSES and _reserve_dispatch(task.id):
