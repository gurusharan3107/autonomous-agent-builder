...
async def run_quality_gates(
    ...
    # Any ERROR (gate infrastructure broken) also halts: post-gates would be
    # untrustworthy, and the dispatch routing must distinguish ERROR (block on
    # config) from FAIL (retry with feedback) — see orchestrator gate routing.
    ...
    if any(r.status == GateStatus.ERROR for r in pre_results):
        return AggregateGateResult(status=GateStatus.ERROR, results=list(pre_results))
    ...
    # Final aggregation. Precedence: FAIL/TIMEOUT (real findings) > ERROR
    # (gate infrastructure broken) > WARN (advisory) > PASS. ERROR must NOT
    # silently coerce to PASS — that's how a misconfigured workspace shipped
    # past quality gates straight to PR creation in the May 2026 sprint-1
    # validation run (see memory: sprint-1-dispatch-exposed-critical-...).
    ...
    if any(r.status == GateStatus.ERROR for r in all_results):
        return AggregateGateResult(status=GateStatus.ERROR, results=all_results)
