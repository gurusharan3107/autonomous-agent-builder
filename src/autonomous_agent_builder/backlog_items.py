...
def feature_to_item_payload(feature: Feature) -> dict[str, Any]:
    item_type = (
        feature.item_type.value if hasattr(feature.item_type, "value") else str(feature.item_type or "feature")
    )
    severity = feature.severity.value if hasattr(feature.severity, "value") else feature.severity
    source = feature.source.value if hasattr(feature.source, "value") else str(feature.source or "manual")
...
def mirror_item_to_artifact(project_root: Path, item: dict[str, Any]) -> None:
    ...
    payload = read_item_artifact(project_root, item_type)
    items = [entry for entry in payload.get(key, []) if isinstance(entry, dict) and entry.get("id") != item.get("id")]
    artifact_item = {
        "id": item.get("id", ""),
