...
from autonomous_agent_builder.claude_runtime import run_claude_prompt
...
"""
    ...
    def validate(self, model: str | None = None) -> AgentQualityGateResult:
        """Run the async quality gate from synchronous callers."""
        try:
            asyncio.get_running_loop()
        except RuntimeError:
            return asyncio.run(self.validate_async(model=model))
        raise RuntimeError(
            "AgentKnowledgeQualityGate.validate() cannot run inside an active event loop; "
            "use `await validate_async(...)` instead."
        )

    async def validate_async(self, model: str | None = None) -> AgentQualityGateResult:
        ...
        prompt = self.EVALUATION_PROMPT.format(
            ...
            expected_docs=", ".join(self._expected_docs()),
        )
        ...
        try:
            ...
            fallback = KnowledgeQualityGate(self.kb_path, self.workspace_path).validate()
            fallback_reason = str(e)

            # Fall back to deterministic validation when the agent lane is unavailable.
            return AgentQualityGateResult(
                passed=fallback.passed,
                score=fallback.score,
                summary=(f"{fallback.summary} (agent-based gate unavailable: {fallback_reason})"),
                evaluation={
                    "fallback": "rule-based",
                ...
                agent_reasoning=(
                    "Agent-based evaluation was unavailable. "
                    f"Rule-based fallback used instead: {fallback_reason}"
                ),
            )

    async def _run_query(self, prompt: str, model: str) -> str:
        """Execute a one-shot Claude prompt and collect its textual output."""
    ...
    def _expected_docs(self) -> list[str]:
        metadata_file = self.kb_path / "extraction-metadata.md"
        if metadata_file.exists():
            frontmatter = self._extract_frontmatter(metadata_file.read_text(encoding="utf-8"))
            expected = frontmatter.get("expected_documents")
            if isinstance(expected, list):
                cleaned = [str(item).strip() for item in expected if str(item).strip()]
                if cleaned:
                    return cleaned
        return [
            "project-overview",
    ...
    def _parse_agent_response(self, response: str) -> AgentQualityGateResult:
        ...
        try:
            ...
            # Build summary
            summary = (
                f"Quality Gate: {'PASSED' if passed else 'FAILED'} (score: {overall_score}/100)"
            )
