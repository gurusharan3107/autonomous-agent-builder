...
BLOCKING_CLAIM_TYPES: frozenset[str] = frozenset({CLAIM_TYPE_EXTRACTED, CLAIM_TYPE_DERIVED})
...
def verify_evidence_manifest(workspace_path: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(claims, list) or not claims:
        ...
        return {
            "valid": False,
            "issues": issues,
            "dependency_hash": manifest.get("dependency_hash"),
        }
...
def build_repo_index(workspace_path: Path, collection_path: Path) -> dict[str, Any]:
    ...
    package_roots = sorted(
        {path.parent.name for path in workspace_path.glob("src/*/__init__.py") if path.is_file()}
    )
...
def citation_for_file(
    workspace_path: Path, relative_path: str, *, kind: str = "evidence"
) -> Citation:
...
def _detect_languages(workspace_path: Path) -> set[str]:
    ...
    if (workspace_path / "pyproject.toml").exists() or (
