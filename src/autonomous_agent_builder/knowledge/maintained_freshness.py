...
def commit_is_reachable_from_ref(
    workspace_path: Path, commit: str, ref: str = CANONICAL_DOC_REF
) -> bool:
    if not commit.strip():
        return False
    try:
...
def baseline_required_for_doc(
    metadata: dict[str, Any], *, created: str | None = None, updated: str | None = None
) -> bool:
    latest = (
        updated
        or created
        or str(metadata.get("updated", "") or "")
        or str(metadata.get("created", "") or "")
    )
