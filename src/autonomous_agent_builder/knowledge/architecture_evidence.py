...
class ArchitectureEvidencePlanner:
    ...
    def _collect_edges(self, nodes: list[GraphNode]) -> list[GraphEdge]:
        edges = [
            ...
            GraphEdge(
                ...
                kind="invokes",
                metadata={
                    "reason": "builder knowledge extract and validate live under the root CLI"
                },
            ),
            GraphEdge(
        ...
        return [edge for edge in edges if edge.source in known_keys and edge.target in known_keys]
    ...
    def _collect_claims(self, nodes: list[GraphNode]) -> list[Claim]:
        ...
        config_nodes = [
            self._node_index[key] for key in sorted(self._node_index) if key.startswith("config:")
        ]
        ...
        claims: list[Claim] = [
            ...
            Claim(
                ...
                citations=(
                    ...
                    data_models,
                    config,
                    *tuple(node.citation for node in config_nodes),
                ),
            ),
            Claim(
                section="Invariants",
            ...
            Claim(
                ...
                citations=(
                    ...
                    agents,
                    self._citation_for_path(
                        "src/autonomous_agent_builder/knowledge/architecture_evidence.py",
                        "class ArchitectureEvidencePlanner",
                    ),
                    self._citation_for_path(
                        "src/autonomous_agent_builder/knowledge/quality_gate.py",
...
class _Span:
    ...
    def _replace_path(self, path: str) -> Citation:
        return Citation(
            path=path, line_start=self.line_start, line_end=self.line_end, kind="integration"
        )
