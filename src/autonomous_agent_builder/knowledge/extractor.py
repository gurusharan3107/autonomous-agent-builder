...
class KnowledgeExtractor:
    ...
    def extract(self, scope: str = "full") -> dict[str, Any]:
        ...
        if unknown_docs:
            raise ValueError("Unsupported KB document slug(s): " + ", ".join(unknown_docs))
    ...
    def _normalize_doc(self, doc: dict[str, Any]) -> dict[str, Any]:
        ...
        if doc_type == "system-docs":
            ...
            normalized["card_summary"] = (
                "Generated artifact inventory, extraction timing, and what changed in this run."
            )
    ...
    def _frontend_dependencies(self) -> list[str]:
        ...
        if not isinstance(deps, dict):
            return []
        return sorted(str(name) for name in deps)
    ...
    def _grounded_evidence_blocks(self, title: str) -> list[str]:
        ...
        if normalized == "project overview":
            ...
            package_phrase = (
                ", ".join(f"`{name}`" for name in packages[:8]) or "no package directories detected"
            )
            return [
                "### Runtime entrypoints\n\n"
        ...
        if normalized == "code structure":
            packages = self._package_dirs()
            package_lines = "\n".join(
                f"- `{name}`: {self._package_purpose(name)}" for name in packages[:10]
            )
        ...
        if normalized == "configuration":
            ...
            lines = []
            for class_name, prefix, fields in snapshots[:6]:
        ...
        if normalized == "workflows and orchestration":
            ...
            return [
                ...
                + "\n".join(
                    f"- `{state}` requires human resolution or stops automation"
                    for state in blocked[:6]
                ),
    ...
    def _normalize_reverse_engineering_content(self, title: str, content: str) -> str:
        ...
        change_guidance = (
            "Edit the owning surface only, verify the adjacent contracts that this document names, and refresh this record with "
    ...
    def _write_metadata(
        ...
        *,
        ...
        metadata = {
            ...
            "workspace_profile": self._shared_graph.get("workspace_profile")
            if self._shared_graph
            else None,
            "graph_artifact": GRAPH_ARTIFACT_RELATIVE_PATH if self._shared_graph else None,
            "graph_dependency_hash": self._shared_graph.get("dependency_hash")
            if self._shared_graph
            else None,
        }
