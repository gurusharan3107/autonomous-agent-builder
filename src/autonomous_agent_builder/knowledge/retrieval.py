...
# Module-level cache for `load_docs`. Keyed on (scope, resolved_root,
# root_mtime_ns) so an unchanged tree returns the same list without re-walking
# the filesystem, while same-process dashboard instances keep project KB roots
# isolated.
# Council 2026-05-08 — Item 5: per-call full-corpus deserialization was the
# dominant cost of every KB tool turn.
_DOCS_CACHE: dict[tuple[str, str, int], list[dict[str, Any]]] = {}


def _docs_cache_key(scope: str) -> tuple[str, str, int] | None:
    root = knowledge_root(scope).resolve()
    if not root.exists():
        return None
    try:
        return (scope, str(root), root.stat().st_mtime_ns)
    except OSError:
        return None
...
def _tags_from_doc(path: Path, doc_type: str, body: str, metadata: dict[str, Any]) -> list[str]:
    ...
    if path.parent != knowledge_root("global") and len(path.parts) > 1:
        tags.update(
            part for part in path.parts[-3:-1] if part not in {".agent-builder", "knowledge", "raw"}
        )
...
def load_docs(scope: str = "local") -> list[dict[str, Any]]:
    ...
    if cache_key is not None:
        ...
        if cached is not None:
            return cached
        # Drop snapshots only for this scope/root when that root has changed.
        for stale in [
            k for k in _DOCS_CACHE if k[0] == scope and k[1] == cache_key[1] and k != cache_key
        ]:
    ...
    if scope == "global":
        ...
        for path in root.rglob("*.md"):
            ...
            docs.append(doc)
        docs.sort(
            key=lambda doc: (str(doc.get("date_published") or ""), doc["title"].lower()),
            reverse=True,
        )
        if cache_key is not None:
    ...
    docs.sort(
        key=lambda doc: (str(doc.get("created_at") or ""), doc["title"].lower()), reverse=True
    )
    if cache_key is not None:
        _DOCS_CACHE[cache_key] = docs
    return docs
