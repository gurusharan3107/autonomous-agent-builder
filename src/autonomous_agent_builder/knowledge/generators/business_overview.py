...
class BusinessOverviewGenerator(BaseGenerator):
    ...
    def generate(self, scope: str = "full") -> dict[str, Any] | None:
        ...
        if business_rules:
            invariants.append(
                "- Refresh rule descriptions when validators, constraints, or policy checks change so the inferred domain guidance stays current."
            )
        ...
        if business_rules:
            ...
            for rule in business_rules[:10]:
                evidence_lines.append(
                    f"- **{rule['name']}**: {rule.get('description', '')}".rstrip()
                )
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.ClassDef):
                            ...
                            if (
                                ...
                                or class_name in ["Base", "Model", "Entity", "BaseModel"]
                                or "Test" in class_name
                                or "Mock" in class_name
                            ):
                                continue
    ...
    def _extract_domain_concepts(self) -> list[str]:
        ...
        if self._is_python_project():
            ...
            for py_file in py_files[:30]:  # Limit to first 30 files
                ...
                if any(
                    skip in py_file.name.lower()
                    for skip in ["test", "migration", "__pycache__", "alembic"]
                ):
                    continue
