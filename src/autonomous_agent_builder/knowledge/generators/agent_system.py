...
- **Write tools**: Write, Edit (workspace plus approved scratch space)
- **Execution tools**: Bash (repo-validated command shape)
- **MCP tools**: Custom tools via Model Context Protocol

## Security Constraints

- Workspace boundary enforcement (PreToolUse)
- Scratch-space writes allowed in system temp dirs including `/tmp`
- Direct KB writes denied; publish through the builder KB lane
- Bash command validation (no shell metacharacters or shell chaining)
...
"""
        ...
        return {
            "title": "Agent System",
            "content": content,
            "tags": ["agents", "tools", "hooks", "security", "system-docs", "seed"],
                    ...
                    if isinstance(node, ast.AsyncFunctionDef):
                        ...
                        if "hook" in node.name.lower() or any(
                            ...
                            if "pretooluse" in node.name.lower() or "pre_tool" in node.name.lower():
                                ...
                                "posttooluse" in node.name.lower()
                                or "post_tool" in node.name.lower()
                            ):
