    ...
    def generate(self, scope: str = "full") -> dict[str, Any] | None:
        ...
        if languages:
            overview_parts.append(
                "The current implementation is primarily written in " + ", ".join(languages) + "."
            )
        if frameworks:
            overview_parts.append(
                "Detected frameworks and delivery layers include " + ", ".join(frameworks) + "."
            )
        overview = " ".join(
            part.strip().rstrip(".") + "." for part in overview_parts if part and part.strip()
        )
                ...
                if content:
                    ...
                    for line in lines:
                        if any(
                            ...
                            for keyword in ["quick start", "getting started", "installation"]
                        ):
