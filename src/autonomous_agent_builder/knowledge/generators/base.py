...
class BaseGenerator(ABC):
    ...
    def __init__(self, workspace_path: Path, output_path: Path | None = None):
        ...
        Args:
            ...
            output_path: Output collection path when a generator needs sidecar artifacts
        """
        self.workspace_path = workspace_path
        self.output_path = output_path
        self.shared_graph: dict[str, Any] | None = None
            ...
            for path in self.workspace_path.rglob(pattern):
                ...
                try:
                    ...
                    if (
                        depth <= max_depth
                        and path.is_file()
                        and not self._is_excluded_path(relative)
                    ):
                        files.append(path)
                except ValueError:
                    continue
    ...
    def _is_excluded_path(self, relative_path: Path) -> bool:
        """Return True when a path points at generated, vendored, or cache surfaces."""
        return any(
            part in self.EXCLUDED_PATH_PARTS or part.startswith(".agent-builder.archive-")
