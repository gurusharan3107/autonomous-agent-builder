    ...
    *,
    ...
    for doc_id in required_docs:
        ...
        if metadata.get("refresh_required") is True and not parsed.updated:
            issues.append(
                f"system doc marked refresh_required but missing updated timestamp: {doc_id}"
            )
            stale_docs.append(doc_id)
        ...
        if feature_id:
            if any(
                value not in {"", feature_id} for value in (linked_feature, recorded_feature_id)
            ):
                issues.append(f"system doc linked to a different feature: {doc_id}")
                mismatched_docs.append(doc_id)
            if not any(
                value == feature_id for value in (linked_feature, recorded_feature_id)
            ) and recorded_task_id not in {
                "",
                task_id or "",
            }:
