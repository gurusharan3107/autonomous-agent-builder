...
class KnowledgeQualityGate:
    ...
    def validate(self) -> QualityGateResult:
        ...
        if passed:
            summary = "Deterministic KB validation passed for blocking docs: " + ", ".join(
                blocking_docs
            )
        else:
            summary = "Deterministic KB validation failed for blocking docs: " + ", ".join(
                blocking_docs
            )
            if claim_failures:
    ...
    def _check_completeness(self) -> QualityCheck:
        blocking_docs = self._blocking_docs()
        missing_docs = [
            slug for slug in blocking_docs if not (self.kb_path / f"{slug}.md").exists()
        ]
    ...
    def _check_claim_validation(self) -> QualityCheck:
        ...
        return QualityCheck(
            ...
            score=1.0
            if issues == 0
            else max(0.0, 1.0 - (issues / max(1, len(self._blocking_docs()) * 4))),
            message=(
    ...
    def _check_freshness(self) -> QualityCheck:
        ...
        for doc_slug in self._blocking_docs():
            ...
            dependency_hash = compute_dependency_hash(
                self.workspace_path, [str(path) for path in dependencies]
            )
        ...
        for doc_path in sorted(self.kb_path.glob("*.md")):
            ...
            if frontmatter.get("refresh_required") is True and not frontmatter.get("updated"):
                issues.append(
                    f"{doc_path.stem}: refresh_required system doc is missing updated timestamp"
                )
            if (doc_type == "testing" or family == "testing") and not frontmatter.get(
                "last_verified_at"
            ):
                issues.append(f"{doc_path.stem}: testing doc is missing last_verified_at")
            if (
                not frontmatter.get("linked_feature")
                and not frontmatter.get("feature_id")
                and not frontmatter.get("task_id")
            ):
                issues.append(
                    f"{doc_path.stem}: maintained system doc is missing feature or task linkage"
                )
    ...
    def _check_searchability(self) -> QualityCheck:
        doc_files = [
            path for path in self.kb_path.glob("*.md") if path.stem != "extraction-metadata"
        ]
        if not doc_files:
            return QualityCheck(
                name="searchability",
        ...
        return QualityCheck(
            ...
            message="Documents have usable tag coverage"
            if avg_tags >= 2.0
            else "Documents need better tag coverage",
    ...
    def _validate_blocking_doc(self, doc_slug: str) -> dict[str, list[dict[str, Any]]]:
        ...
        for unresolved in manifest.get("unresolved_claims") or []:
            unresolved_claims.append(
                self._normalize_manifest_issue(doc_slug, unresolved, reason="unresolved_claim")
            )
        for contradicted in manifest.get("contradicted_claims") or []:
            contradicted_claims.append(
                self._normalize_manifest_issue(doc_slug, contradicted, reason="contradicted_claim")
            )
    ...
    def _normalize_manifest_issue(
        ...
        *,
        ...
        normalized = dict(issue) if isinstance(issue, dict) else {"claim": str(issue)}
    ...
    def _generated_docs(self) -> set[str]:
        return {
            path.stem for path in self.kb_path.glob("*.md") if path.stem != "extraction-metadata"
        }
    ...
    def _blocking_render_status(self) -> dict[str, Any]:
        ...
        for doc_slug in self._blocking_docs():
            ...
            statuses[doc_slug] = (
                status if isinstance(status, dict) else {"rendered_from_graph": False}
            )
        return statuses
