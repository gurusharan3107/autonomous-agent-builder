...
def _resolve_kb_relative_path(
    ...
    if relative_path.is_absolute():
        raise PublishError(f"{label} must be a relative KB path.")
    if any(part == ".." for part in normalized.split("/")):
        raise PublishError(f"{label} cannot contain parent traversal.")

    resolved_path = resolve_contained_path(root, relative_path)
    if resolved_path is None:
        raise PublishError(f"{label} escapes the KB root.")
...
def _persist_document(
    *,
    ...
    if existing_doc_id:
        relative_path, path = _resolve_kb_relative_path(
            root,
            existing_doc_id,
            label="existing_doc_id",
        )
    else:
...
def _resolve_doc_path(doc_id: str, scope: str) -> Path:
    ...
    _, path = _resolve_kb_relative_path(root, doc_id, label="doc_id")
    if not path.exists():
...
def _target_relative_path(
    *,
    ...
    directory = (
        Path("raw")
        if scope == "global"
        else Path(collection or _default_collection_for_doc_type(doc_type))
    )
    if file_name:
