...
SUPPRESSED_API_KEY_ENV_KEYS = ("ANTHROPIC_API_KEY",)
...
def _onecli_url() -> str:
    return (
        os.environ.get("ONECLI_URL") or os.environ.get("AAB_ONECLI_URL") or "http://127.0.0.1:10254"
    ).rstrip("/")
...
def scrub_provider_env(env: dict[str, str]) -> dict[str, str]:
    """Suppress API-key auth while preserving local Claude OAuth auth.

    The builder's local Claude Agent SDK lane is subscription/OAuth backed.
    `ANTHROPIC_API_KEY` must not become the default auth path, while
    `CLAUDE_CODE_OAUTH_TOKEN` is allowed so local SDK child processes can use
    the operator's Claude subscription token.
    """
    return {
        key: ("placeholder" if key in SUPPRESSED_API_KEY_ENV_KEYS else value)
