    ...
    async with engine.begin() as conn:
        ...
        if engine.dialect.name == "sqlite":
            ...
            if "repo_identity" not in columns:
                await conn.execute(text("ALTER TABLE chat_sessions ADD COLUMN repo_identity VARCHAR(1024)"))
            if "workspace_cwd" not in columns:
                await conn.execute(text("ALTER TABLE chat_sessions ADD COLUMN workspace_cwd VARCHAR(1024)"))
            ...
            if "item_type" not in feature_columns:
                await conn.execute(text("ALTER TABLE features ADD COLUMN item_type VARCHAR(32) DEFAULT 'feature'"))
            ...
            if "source" not in feature_columns:
                await conn.execute(text("ALTER TABLE features ADD COLUMN source VARCHAR(32) DEFAULT 'manual'"))
            ...
            if "acceptance_criteria" not in feature_columns:
                await conn.execute(text("ALTER TABLE features ADD COLUMN acceptance_criteria JSON DEFAULT '[]'"))
            if "dependencies" not in feature_columns:
                await conn.execute(text("ALTER TABLE features ADD COLUMN dependencies JSON DEFAULT '[]'"))
            ...
            if "output_text" not in run_columns:
                await conn.execute(text("ALTER TABLE agent_runs ADD COLUMN output_text TEXT DEFAULT ''"))
            if "runtime_sdk" not in run_columns:
                await conn.execute(text("ALTER TABLE agent_runs ADD COLUMN runtime_sdk VARCHAR(50) DEFAULT ''"))
            if "provider" not in run_columns:
                await conn.execute(text("ALTER TABLE agent_runs ADD COLUMN provider VARCHAR(100) DEFAULT ''"))
            if "model" not in run_columns:
                await conn.execute(text("ALTER TABLE agent_runs ADD COLUMN model VARCHAR(100) DEFAULT ''"))
