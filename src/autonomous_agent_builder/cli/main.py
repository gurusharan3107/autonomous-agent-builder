...
# Load Builder-owned env for standalone builder CLI commands. Generated app
# `.env` files must not own Autonomous Agent Builder secrets or runtime config.
load_dotenv(builder_source_env_path())
            ...
            if use_json:
                emit_error(
                    ...
                    detail={
                        ...
                        "message": sanitize_error_detail(str(exc)),
                    },
                    exit_code=1,
                    use_json=True,
                )
...
def _doctor_payload() -> dict[str, object]:
    ...
    readiness_payload = (
        compact_status(load_readiness_status(project_root))
        if project_initialized
            ...
            "blocking_reasons": [],
            "invalidated_by": [],
            "next": [{"code": "run_init", "command": "builder init", "reason": project_hint}],
        }
    )
...
def doctor(
    ...
    def fmt(data: dict[str, object]) -> str:
        ...
        readiness_info = checks.get("readiness", {"state": "unknown"})
        lines = [
            ...
            f"readiness_state: {readiness_info['state']}",
...
app.command(name="start", help="Build/publish the dashboard, then start the local product.")(
    start_command
)

# ── Operator Entry Points ──
app.command(name="map", help="Bounded workspace digest for startup orientation.")(map.map_command)
app.command(name="context", help="Task bootstrap context for named profiles.")(
    context.context_command
)
app.command(name="lint", help="Fast structural lint across builder product surfaces.")(
    lint.lint_command
)
app.command(name="quality-gate", help="Local quality-gate surfaces.")(gate.quality_gate_command)
app.command(name="verify", help="Plan or execute behavioral proof for builder changes.")(
...
app.add_typer(readiness.app, name="readiness", help="Day-0 readiness contract and gating.")
app.add_typer(server.app, name="server", help="Builder-owned local server lifecycle.")
