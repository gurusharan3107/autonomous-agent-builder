...
MEMORY_REQUIRED_TEMPLATE_SECTIONS = (
...
MEMORY_PRIMARY_SECTION_BY_TYPE = {
    "decision": "Decision",
    "pattern": "Pattern",
...
def _has_markdown_heading(body: str, heading: str) -> bool:
    expected = f"## {heading}".casefold()
    return any(line.strip().casefold() == expected for line in body.splitlines())


def _missing_template_sections(entry_type: str, body: str) -> list[str]:
    missing: list[str] = []
    primary = MEMORY_PRIMARY_SECTION_BY_TYPE.get(entry_type)
    if primary and not _has_markdown_heading(body, primary):
        missing.append(primary)
    for section in MEMORY_REQUIRED_TEMPLATE_SECTIONS:
        if not _has_markdown_heading(body, section):
...
def _memory_contract_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "rules": [
            ...
            "Prefer an Agent Retrieval Summary near the top so future agents know when and how to apply the memory.",
            "Prefer a User-Facing Summary when the memory is displayed in the dashboard.",
            "Include Retrieval Queries for memories that should be found across future sessions.",
        ...
        "sample_markdown": _build_memory_markdown(
            ...
            (
                ...
                "## Agent Retrieval Summary\n\n"
                "Retrieve this memory when a future agent is choosing between repo-local builder commands and global workflow surfaces.\n\n"
                "Use this as the operating rule: project-specific precedent belongs in `builder memory`; global doctrine belongs in `workflow` or the global control plane.\n\n"
                "## User-Facing Summary\n\n"
                "This memory explains where project-specific lessons should be stored so future runs do not rediscover the same issue.\n\n"
                "## Reusable Guidance\n\n"
                "- Use `builder memory add` for repo-local decisions, patterns, and corrections.\n"
                "- Keep the memory narrow enough to apply in future sessions.\n"
                "- Do not duplicate global doctrine in a project memory.\n\n"
                "## When To Apply\n\n"
                "Apply this when closing a validation run, recording a repeated correction, or preserving a non-obvious project decision.\n\n"
                "## Retrieval Queries\n\n"
                "- repo memory owner surface\n"
                "- builder memory add format\n"
                "- project-specific precedent\n"
...
def _compact_memory_contract_payload(payload: dict[str, Any]) -> dict[str, Any]:
    add_command = "builder memory add --type decision --phase <phase> --entity <surface> --title <title> --content-file <file> --json"
    return {
        "doc_type": payload.get("doc_type", "memory"),
        "memory_root": payload.get("memory_root", ""),
        "required_frontmatter": payload.get("required_frontmatter", {}),
        "optional_frontmatter_keys": sorted((payload.get("optional_frontmatter") or {}).keys()),
        "allowed_types": payload.get("allowed_types", []),
        "allowed_statuses": payload.get("allowed_statuses", []),
        "recommended_body_sections": payload.get("recommended_body_sections", []),
        "rules": payload.get("rules", [])[:4],
        "actionable_next": add_command,
        "next_step": "builder memory lint --json",
        "progressive_disclosure": [
            {
                "when": "create a memory using the contract",
                "command": add_command,
            },
            {
                "when": "validate existing memory files",
                "command": "builder memory lint --json",
            },
            {
                "when": "inspect sample markdown and full lifecycle rules",
                "command": "builder memory contract --json --full",
            },
        ],
        "diagnostics": {
            "full_payload_command": "builder memory contract --json --full",
            "contains": ["sample_markdown", "full lifecycle rules", "frontmatter type map"],
        },
    }
...
def _lint_memory_entries(root: Path | None = None) -> dict[str, Any]:
    ...
    for path in _memory_markdown_files(memory_root):
        ...
        if legacy_without_frontmatter:
            if not body.strip():
                issues.append(
                    {"path": relative, "severity": "error", "message": "missing frontmatter"}
                )
                continue
            issues.append(
                {
        ...
        required_keys = (
            ("title", "type", "status")
            if legacy_without_frontmatter
            else MEMORY_CONTRACT_REQUIRED_KEYS
        )
                ...
                issues.append(
                    {
                        "path": relative,
                        "severity": "error",
                        "message": f"missing required frontmatter '{key}'",
                    }
                )
            ...
            issues.append(
                {
                    "path": relative,
                    "severity": "error",
                    "message": f"invalid memory type '{entry_type}'",
                }
            )
        ...
        if not body.strip():
            issues.append(
                {"path": relative, "severity": "error", "message": "missing markdown body"}
            )
        elif not legacy_without_frontmatter and status == "active":
            for section in _missing_template_sections(entry_type, body):
                issues.append(
                    {
                        "path": relative,
                        "severity": "error",
                        "message": f"active memory missing required template section '## {section}'",
                    }
                )
...
def _post_mutation_check(slug: str | None = None) -> dict[str, Any]:
    """Rebuild memory routing and prove the mutation left memory retrievable."""
    entries = _scan_filesystem()
    _save_routing(entries)
    lint_payload = _lint_memory_entries()
    resolved = _resolve_entry(slug, entries) if slug else None
    retrieval_checked = bool(slug)
    retrieval_passed = bool(resolved) if retrieval_checked else True
    return {
        "reindexed": True,
        "memory_count": len(entries),
        "lint_passed": bool(lint_payload.get("passed")),
        "error_count": int(lint_payload.get("error_count", 0)),
        "warning_count": int(lint_payload.get("warning_count", 0)),
        "retrieval_checked": retrieval_checked,
        "retrieval_passed": retrieval_passed,
        "retrieved_slug": resolved.get("slug") if resolved else "",
        "issues": lint_payload.get("issues", [])[:5],
    }


def _post_mutation_exit_code(post_mutation: dict[str, Any] | None) -> int:
    if not post_mutation:
        return EXIT_SUCCESS
    if not post_mutation.get("lint_passed", False):
        return 1
    if post_mutation.get("retrieval_checked") and not post_mutation.get("retrieval_passed", False):
        return 1
    return EXIT_SUCCESS
...
@app.command()
def contract(
    full: bool = typer.Option(False, "--full", help="Include sample markdown and full rules."),
    json: bool = typer.Option(False, "--json", help="Output as JSON."),
) -> None:
    """Show the canonical memory markdown contract and sample entry."""
    full_payload = _memory_contract_payload()
    payload = full_payload if full else _compact_memory_contract_payload(full_payload)
...
def search(
    ...
    compact = [
        _memory_compact(entry, preview=str(entry.get("match_preview", ""))) for entry in results
    ]
    render(
...
def add(
    ...
    post_mutation = _post_mutation_check(slug)
    ...
    render(payload, fmt, use_json=json)
    sys.exit(_post_mutation_exit_code(post_mutation))
...
def stats(
    ...
    def fmt(data: dict[str, Any]) -> str:
        ...
        if data["types"]:
            lines.append(
                "types: " + ", ".join(f"{k}={v}" for k, v in sorted(data["types"].items()))
            )
        if data["statuses"]:
            lines.append(
                "statuses: " + ", ".join(f"{k}={v}" for k, v in sorted(data["statuses"].items()))
            )
        return "\n".join(lines)
...
def _mutate_entry(slug: str, mutator, *, json: bool) -> None:
    ...
    post_mutation: dict[str, Any] | None = None
    if changed:
        _write_entry_file(entry, body)
        post_mutation = _post_mutation_check(entry["slug"])

    payload = {"status": "ok" if changed else "unchanged", "memory": entry, "changed": changed}
    if post_mutation:
        payload["post_mutation"] = post_mutation

    render(payload, lambda data: f"{data['status']}: {entry['slug']}", use_json=json)
    sys.exit(_post_mutation_exit_code(post_mutation))
...
def relate(
    ...
    if changed:
        ...
        post_mutation = _post_mutation_check(source["slug"])
    ...
    render(
        payload,
        lambda _: f"{payload['status']}: {source['slug']} -> {target['slug']}",
        use_json=json,
    )
    sys.exit(_post_mutation_exit_code(post_mutation))
