...
def _run_list(client, *, task: str | None = None, limit: int | None = None) -> list[dict[str, Any]]:
    ...
    if not items and task is None:
        items = _run_list_from_metrics(client)
    return items[:limit] if limit else items


def _run_list_from_metrics(client) -> list[dict[str, Any]]:
    """Fallback to dashboard metrics when the dedicated run index is empty."""
    try:
        payload = client.get("/dashboard/metrics")
    except AabApiError:
        return []
...
def _run_resolution(query: str, client, *, task: str | None = None) -> RetrievalResolution:
    ...
    if resolution.item:
        run_id = str(resolution.item.get("id", "")).strip()
        if not run_id:
            return resolution
        full_item = resolution.item
        try:
            full_item = client.get(f"/runs/{run_id}")
        except AabApiError:
            # Metrics payloads already carry the compact run fields needed by the CLI.
            full_item = resolution.item
...
def list_runs(
    ...
    try:
        ...
        render(
            compact_results_payload(
                "list", compact, next_step="builder backlog run summary <query> --json"
            )
            if json
            else compact,
            fmt,
...
def search(
    ...
    try:
        ...
        render(
            compact_results_payload(
                query, compact, next_step="builder backlog run summary <query> --json"
            )
            if json
            else compact,
            fmt,
