...
from autonomous_agent_builder.cli.local_fallback import load_local_task_status
...
def _task_list(
    client, *, feature: str | None = None, limit: int | None = None
) -> list[dict[str, Any]]:
    data = client.get(f"/features/{feature}/tasks") if feature else client.get("/tasks")
...
def _task_compact(item: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "preview": make_preview(
            item, preview_keys=("description", "blocked_reason"), max_chars=120
        ),
    }
...
def list_tasks(
    ...
    try:
        ...
        def fmt(rows: list[dict[str, Any]]) -> str:
            ...
            table_rows = [
                [row["id"], row["title"], format_status(row["status"]), row["preview"]]
                for row in rows
            ]
        ...
        render(
            compact_results_payload(
                "list", compact, next_step="builder backlog task summary <query> --json"
            )
            if json
            else compact,
            fmt,
...
def search(
    ...
    try:
        ...
        render(
            compact_results_payload(
                query, compact, next_step="builder backlog task summary <query> --json"
            )
            if json
            else compact,
            fmt,
...
def status(
    ...
    try:
        try:
            data = request_json(client, "GET", f"/tasks/{task_id}")
        except BuilderConnectivityError:
            data = load_local_task_status(task_id)
            if data is None:
                emit_error(
                    "task not found in local board state",
                    code="not_found",
                    hint="Run 'builder board show --json', then retry with an exact task ID.",
                    use_json=json,
                )
                sys.exit(1)
    except AabApiError as e:
...
def dispatch(
    ...
    yes: bool = typer.Option(False, "--yes", "-y", help="Skip confirmation and dispatch."),
    json: bool = typer.Option(False, "--json", help="Output as JSON."),
) -> None:
