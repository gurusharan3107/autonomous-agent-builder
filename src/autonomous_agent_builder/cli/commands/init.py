...
def init_command(
    ...
    language: str = typer.Option("python", "--language", help="Primary language (python, node, java, go)."),
    ...
    try:
        ...
        def fmt(d: dict) -> str:
            ...
            lines = [
                f"✓ Initialized agent builder in {d['directory']}",
                f"  Project: {d['project_name']}",
                f"  Language: {d['language']}",
                f"  Framework: {d.get('framework', 'none')}",
                "",
                "Next steps:",
                "  1. builder start          # Start the embedded server",
                "  2. Open http://localhost:8000  # Access the dashboard",
            ]
            return "\n".join(lines)
