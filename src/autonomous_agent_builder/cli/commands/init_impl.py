...
def _resolve_language(project_root: Path, requested_language: str | None) -> str:
    ...
    if detected == "unknown":
        # Keep historical default behavior when no repository markers exist.
        return "python"
    return detected
...
def _copy_embedded_resources(agent_builder_dir: Path) -> None:
    ...
    dashboard_src = live_dashboard_src if live_dashboard_src.exists() and any(live_dashboard_src.iterdir()) else embedded_dir / "dashboard"
