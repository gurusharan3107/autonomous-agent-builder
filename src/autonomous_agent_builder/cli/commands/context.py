...
CONTEXT_PROFILES: dict[str, dict[str, object]] = {
    "repo-init": {
        ...
        "success_criteria": [
            ...
            "readiness state is agent_ready or reports a deterministic next action",
    ...
    "readiness": {
        ...
        "summary": "Assess whether an initialized repo can safely hand work to an autonomous agent.",
        "commands": [
            "builder readiness status --json",
            "builder readiness assess --json",
            "builder --json doctor",
            "builder context task-dispatch --json",
        ],
        "success_criteria": [
            "readiness state is agent_ready before autonomous continuation",
            "missing, stale, or blocked state returns an exit code and next action",
            "forward and reverse engineering modes report the expected required checks",
        ],
    },
    "system-docs": {
        "title": "System docs",
...
def context_command(
    task: str = typer.Argument(
        ...,
        help="Task profile: repo-init, readiness, system-docs, task-dispatch, verification.",
    ),
