...
def _handle_project_lookup_error(
    query: str, resolution: RetrievalResolution, *, use_json: bool
) -> None:
    hint = not_found_hint(
        query,
...
def list_projects(
    ...
    try:
        ...
        render(
            compact_results_payload(
                "list", compact, next_step="builder backlog project summary <query> --json"
            )
            if json
            else compact,
            fmt,
...
def search(
    ...
    try:
        ...
        render(
            compact_results_payload(
                query, compact, next_step="builder backlog project summary <query> --json"
            )
            if json
            else compact,
            fmt,
