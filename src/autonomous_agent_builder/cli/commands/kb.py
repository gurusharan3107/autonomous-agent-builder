...
app = typer.Typer(help="Project-local knowledge surfaces for agents and operators.")
...
def _summarize_document(doc: dict[str, Any]) -> str:
    ...
    if invariants:
        invariant_lines = [
            line.strip() for line in invariants.splitlines() if line.strip().startswith("-")
        ][:3]
        if invariant_lines:
...
def _run_extract_pipeline(
    *,
    ...
    agent_payload = default_agent_advisory_payload()

    required_error_slugs = {doc_slug} if doc_slug else set(get_settings().kb_blocking_docs)
    blocking_errors = [
        error_item
        for error_item in errors
        if str(error_item.get("slug", "")).strip() in required_error_slugs
    ]
...
def add(
    ...
    payload = {
        ...
        "extra_fields": _system_doc_extra_fields(
            ...
            owned_paths=[part.strip() for part in (owned_paths or "").split(",") if part.strip()]
            or None,
...
def list_docs(
    ...
    full: bool = typer.Option(
        False, "--full", help="Include complete document payloads in JSON output."
    ),
    json: bool = typer.Option(False, "--json", help="Output as JSON."),
) -> None:
    ...
    items = items[:limit]
    payload = (
        _kb_full_payload("list", items) if json and full else _kb_search_payload("list", items)
    )
...
def extract(
        ...
        if doc and target_file is None:
            ...
            if json_output:
                payload = _build_extract_contract(
                    ...
                    passed=False,
                    operator_message=(
                        f"Knowledge already extracted at {kb_path}. Use --force to regenerate."
                    ),
                    next_step=_kb_extract_next_step(
                        action="stop",
    ...
    try:
        ...
        if validate:
            ...
            typer.echo(f"  • Validation: {output_data['validation']['deterministic']['summary']}")
...
def validate(
    ...
    json_output: bool = typer.Option(False, "--json", help="Output as JSON"),
    full: bool = typer.Option(False, "--full", help="Include full validation evidence."),
    ...
    try:
        ...
        advisory_payload = default_agent_advisory_payload()
        ...
        if json_output:
            full_payload = validate_output_payload(
                deterministic_result,
                agent_advisory=advisory_payload,
            )
            payload = full_payload if full else compact_validate_output_payload(full_payload)
            render_json(payload)
        else:
                        ...
                        if isinstance(value, list) and value:
                            ...
                            for item in value[:5]:
                                if key == "maintained_docs" and isinstance(item, dict):
                                    ...
                                    if matched:
                                        typer.echo(
                                            f"          matched_changed_paths: {', '.join(str(path) for path in matched[:4])}"
                                        )
                                else:
                                    typer.echo(f"        - {item}")
                            if len(value) > 5:
            ...
            if not deterministic_result.passed:
                typer.echo("\nNext step:")
                typer.echo(
                    "  • Run `builder knowledge extract --force --json` after repairing the cited blocking docs."
                )
