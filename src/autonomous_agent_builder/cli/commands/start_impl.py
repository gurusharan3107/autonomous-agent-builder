...
def run_start(
    ...
    """
    _load_start_env(agent_builder_dir)
...
def _load_start_env(agent_builder_dir: Path) -> list[Path]:
    """Load Builder-owned config/secrets from the source `.env` only."""

    loaded: list[Path] = []
    source_env = builder_source_env_path()
    if source_env.exists():
        load_dotenv(source_env, override=False)
        loaded.append(source_env)
    return loaded
