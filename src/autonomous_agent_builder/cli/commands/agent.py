...
def _compact_session(item: dict[str, Any]) -> dict[str, Any]:
    compact = {
        "id": str(item.get("id", "")),
        "sdk_session_id": item.get("sdk_session_id"),
        "updated_at": str(item.get("updated_at", "")),
        "message_count": item.get("message_count", 0),
        "preview": truncate(str(item.get("preview", "") or ""), 120),
    }
    if item.get("created_at"):
        compact["created_at"] = str(item.get("created_at", ""))
    if item.get("status"):
        compact["status"] = item.get("status")
    return compact


def _compact_sessions_payload(
    data: dict[str, Any], items: list[Any], *, limit: int
) -> dict[str, Any]:
    sessions = [_compact_session(item) for item in items if isinstance(item, dict)]
    next_session = sessions[0]["id"] if sessions else "<id>"
    payload = {
        "status": "ok",
        "count": len(sessions),
        "limit": limit,
        "results": sessions,
        "next_step": f"builder agent history --session {next_session} --json",
        "actionable_next": f"builder agent history --session {next_session} --json",
        "progressive_disclosure": [
            {
                "reveals": "latest session transcript summary",
                "command": f"builder agent history --session {next_session} --json",
            },
            {
                "reveals": "visible timeline items for one session",
                "command": f"builder agent history --session {next_session} --full --json",
            },
            {
                "reveals": "raw session-list fields returned by the API or local DB",
                "command": "builder agent sessions --json --full",
            },
        ],
    }
    for key in ("degraded", "source", "repo_identity", "workspace_cwd"):
        if key in data:
...
def list_sessions(
    limit: int = typer.Option(5, help="Max sessions."),
    full: bool = typer.Option(False, "--full", help="Include raw session-list fields."),
    ...
    try:
        ...
        if full:
            ...
            payload = _compact_sessions_payload(
...
def meta(
    ...
    try:
        ...
        render(
            payload,
            lambda item: f"model: {item.get('model', '')}\nNext: {item.get('next_step', '')}",
            use_json=json,
        )
        sys.exit(EXIT_SUCCESS)
    finally:
