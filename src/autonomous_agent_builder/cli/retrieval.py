...
def resolve_collection_item(
    ...
    *,
    ...
    for item in items:
        ...
        best_ratio = max(
            [
                SequenceMatcher(a=lowered, b=value).ratio()
                for value in [*lowered_ids, *lowered_texts]
                if value
            ],
            default=0.0,
        )
    ...
    if (
        top_mode in {"id", "name", "prefix"}
        or second_score is None
        or (second_score - top_score) >= 0.2
    ):
        suggestions = [
            suggestion_label(item, id_key=suggestion_id_key, label_key=suggestion_label_key)
            for _, _, item in ranked[1:4]
        ]
        return RetrievalResolution(
            item=top_item, matched_on=top_mode, suggestions=[s for s in suggestions if s]
        )
