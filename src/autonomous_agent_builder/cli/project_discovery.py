...
def _is_builder_source_repo(root: Path) -> bool:
    """Return whether ``root`` is the autonomous builder source checkout/worktree."""
    pyproject = root / "pyproject.toml"
    runtime_package = root / "src" / "autonomous_agent_builder" / "cli" / "main.py"
    if not pyproject.is_file() or not runtime_package.is_file():
        return False
    try:
        contents = pyproject.read_text(encoding="utf-8")
    except OSError:
        return False
    return 'name = "autonomous-agent-builder"' in contents


def _reject_builder_source_repo(root: Path) -> None:
    if _is_builder_source_repo(root):
        raise ProjectNotFoundError(
            message="Builder source repository is not a builder-managed app project",
            hint=(
                "Run builder commands from the generated app workspace. The autonomous "
                "builder repo/worktree itself must not be initialized with .agent-builder."
            ),
        )
...
def find_agent_builder_dir(start_path: Path | None = None) -> Path:
    ...
    while True:
        _reject_builder_source_repo(current)
