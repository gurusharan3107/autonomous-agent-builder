...
def _serialize_doc(path: Path, scope: str) -> dict[str, Any]:
    ...
    created_at = (
        metadata.get("date_published") or metadata.get("date_processed") or parsed.created or ""
    )
...
def _global_doc_paths() -> list[Path]:
    ...
    for article in _global_routing_articles():
        ...
        if not file_name:
            continue
        candidate = resolve_contained_path(raw_root, file_name)
...
async def list_tags(
    ...
    tag_docs = {tag: [doc for doc in docs if tag in doc.get("tags", [])] for tag in counter}
    ...
    for tag, count in counter.most_common():
        related_counts = Counter(
            other for doc in tag_docs[tag] for other in doc.get("tags", []) if other != tag
        )
        payload.append(
            {
                "name": tag,
