...
def _item_enums(data: BacklogItemCreate) -> tuple[BacklogItemType, list[str], BacklogItemSeverity | None, BacklogItemSource]:
...
def _update_item_enums(
    ...
    severity_input = data.severity if data.severity is not None else (
    ...
    source = normalize_source(
        data.source if data.source is not None else (
            item.source.value if hasattr(item.source, "value") else item.source
        )
    )
...
async def _create_backlog_item(
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail={"code": "invalid_backlog_item", "message": str(exc)}) from exc
...
@router.post("/projects/{project_id}/backlog/items", response_model=BacklogItemResponse, status_code=201)
...
async def list_backlog_items(
    ...
    if item_type:
        try:
            query = query.where(Feature.item_type == BacklogItemType(normalize_item_type(item_type)))
        except ValueError as exc:
            raise HTTPException(status_code=422, detail={"code": "invalid_backlog_item", "message": str(exc)}) from exc
