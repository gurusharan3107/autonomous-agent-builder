...
class CodexCliRuntime(AgentRuntime):
    ...
    def __init__(
        ...
        *,
        ...
        if sdk_name == "codex_sdk":
            raise ValueError(
                "codex_sdk uses CodexAppServerRuntime; use codex_cli for codex exec."
            )
    ...
    async def run(
        ...
        *,
        ...
        for attempt in range(attempts):
            ...
            final_output = (
                final_output
                or _event_final_output(raw_events)
                or "".join(stream_parts).strip()
            )
    ...
    async def probe(self) -> RuntimeProbeResult:
        ...
        return RuntimeProbeResult(
            ...
            message=(
                "Codex CLI is available. ChatGPT subscription auth is managed by "
                "`codex login`."
            ),
            next=(
